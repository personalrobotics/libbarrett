...
class NetworkHaptics
    : public barrett::systems::SingleIO<barrett::units::CartesianPosition::type,
                                        barrett::units::CartesianForce::type> {
	BARRETT_UNITS_FIXED_SIZE_TYPEDEFS;

  public:
	static const int SIZE_OF_MSG = 3 * sizeof(double);

	explicit NetworkHaptics(barrett::systems::ExecutionManager *em,
	                        char *remoteHost, int port_src = 5557,
	                        int port_dest = 5556,
	                        const std::string &sysName = "NetworkHaptics")
	    : barrett::systems::SingleIO<cp_type, cf_type>(sysName), sock(-1),
	      cp(0.0), numMissed(0), cf(0.0) {
		int err;
		long flags;
		int buflen;
		...
		if (sock == -1) {
			(barrett::logMessage("(NetworkHaptics::NetworkHaptics): Ctor "
			                     "failed %s: Could not create socket.") %
			 __func__)
		...
		if (flags < 0) {
			(barrett::logMessage("(NetworkHaptics::NetworkHaptics): Ctor "
			                     "failed  %s: Could not get socket flags.") %
			 __func__)
			    .raise<std::runtime_error>();
		}
		flags |= O_NONBLOCK;
		...
		if (err < 0) {
			(barrett::logMessage("(NetworkHaptics::NetworkHaptics): Ctor "
			                     "failed %s: Could not set socket flags.") %
			 __func__)
			    .raise<std::runtime_error>();
		}
		...
		err = getsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *)&buflen,
		                 &buflenlen);
		if (err) {
			(barrett::logMessage(
			     "(NetworkHaptics::NetworkHaptics): Ctor failed %s: Could not "
		...
		err =
		    setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *)&buflen, buflenlen);
		if (err) {
			(barrett::logMessage(
			     "(NetworkHaptics::NetworkHaptics): Ctor failed  %s: Could not "
		...
		if (err) {
			(barrett::logMessage("(NetworkHaptics::NetworkHaptics): Ctor "
			                     "failed %s: Bad IP argument '%s'.") %
			 __func__ % remoteHost)
			    .raise<std::runtime_error>();
		}

		/* Call "connect" to set datagram destination */
		err = connect(sock, (struct sockaddr *)&their_addr,
		              sizeof(struct sockaddr));
		if (err) {
			(barrett::logMessage(
			     "(NetworkHaptics::NetworkHaptics): Ctor failed %s: Could not "
			     "set datagram destination.") %
			 __func__)
			    .raise<std::runtime_error>();
		}
