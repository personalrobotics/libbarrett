...
int wam_main(int argc, char **argv, ProductManager &pm,
	...
	if (hand != NULL) {
		...
		if (hand->hasTactSensors()) {
			...
			for (size_t i = 0; i < tps.size(); ++i) {
				drawBoard(stdscr, line, i * TACT_BOARD_STRIDE, TACT_BOARD_ROWS,
				          TACT_BOARD_COLS, TACT_CELL_HEIGHT, TACT_CELL_WIDTH);
			}
		} else {
			printw(" n/a");
		}
	...
	while (pm.getSafetyModule()->getMode() == SafetyModule::ACTIVE) {
		...
		to = wam.getToolOrientation(); // We work only with unit quaternions. No
		                               // saturation necessary.
		mvprintw(line++, wamX, "%+7.4f %+7.4fi %+7.4fj %+7.4fk", to.w(), to.x(),
		         to.y(), to.z());

		R = to.toRotationMatrix(); // convert to a 3x3 rotation matrix
		line++;
		mvprintw(line++, wamX, "[%+7.4f %+7.4f %+7.4f]", R(0, 0), R(0, 1),
		         R(0, 2));
		mvprintw(line++, wamX, "[%+7.4f %+7.4f %+7.4f]", R(1, 0), R(1, 1),
		         R(1, 2));
		mvprintw(line++, wamX, "[%+7.4f %+7.4f %+7.4f]", R(2, 0), R(2, 1),
		         R(2, 2));
		...
		if (fts != NULL) {
			...
			mvprintw(line++, ftsX, "[%6.2f, %6.2f, %6.2f]", cf[0], cf[1],
			         cf[2]);
			ct = math::saturate(fts->getTorque(), 9.999);
			mvprintw(line++, ftsX, "[%6.3f, %6.3f, %6.3f]", ct[0], ct[1],
			         ct[2]);

			fts->updateAccel();
			ca = math::saturate(fts->getAccel(), 99.99);
			mvprintw(line++, ftsX, "[%6.2f, %6.2f, %6.2f]", ca[0], ca[1],
			         ca[2]);
		}
		...
		if (hand != NULL) {
			line = handY;
			// hand->update();  // Update all sensors, includes
			// Hand::S_TACT_FULL, if installed
			hand->update(Hand::S_POSITION + Hand::S_FINGERTIP_TORQUE +
			             Hand::S_TACT_TOP10); // Update these sensors

			hjp = math::saturate(hand->getInnerLinkPosition(), 9.999);
			mvprintw(line++, handX, "[%6.3f, %6.3f, %6.3f, %6.3f]", hjp[0],
			         hjp[1], hjp[2], hjp[3]);
...
void graphPressures(WINDOW *win, int starty, int startx,
	...
	for (int i = 0; i < pressures.size(); ++i) {
		graphCell(win,
		          starty + 1 +
		              TACT_CELL_HEIGHT * (TACT_BOARD_ROWS - 1 -
