...
int wam_main(int argc, char** argv, ProductManager& pm, systems::Wam<DOF>& wam) {
    ...
    LowLevelWam<DOF> &hardware = wam.getLowLevelWam();
    Puck *puck = hardware.getPucks()[index];

    std::cout << "P JP ECMIN ECMAX" << std::endl;

    for (;;) {
#if 1
        double motorPosition = hardware.getJointPositions(LowLevelWam<DOF>::PS_MOTOR_ENCODER)[index];
        double jointPosition = hardware.getJointPositions(LowLevelWam<DOF>::PS_JOINT_ENCODER)[index];
#else
        int motorPosition = puck->getProperty(Puck::P);
        int jointPosition = puck->getProperty(Puck::JP);
#endif
        int ecmin = puck->getProperty(Puck::ECMIN);
        int ecmax = puck->getProperty(Puck::ECMAX);

        std::cout << motorPosition << " " << jointPosition << " " << ecmin << " " << ecmax << std::endl;
    }
