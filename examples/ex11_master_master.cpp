...
int wam_main(int argc, char **argv, ProductManager &pm,
  ...
  while (true) {
    ...
    case 'g':
      if (ghcThread == NULL) {
        if (pm.foundGimbalsHandController()) {
          ghcThread = new boost::thread(ghcEntryPoint,
                                        pm.getGimbalsHandController(), argv[1]);
...
int openSocket(const char *remoteHost, int port = 3333) {
  ...
  if (err) {
    (logMessage("openSocket(): Failed.  %s: Bad IP argument '%s'.") % __func__ %
     remoteHost)
        .raise<std::runtime_error>();
  }
  ...
  while (!boost::this_thread::interruption_requested()) {
    ...
    if (data != data_1) {
      printf("%d,%d  %d,%d  %d,%d  %d,%d  %f\n", ghc->getThumbOpen(),
             ...
             ghc->getMiddleClose(), ghc->getRockerUp(), ghc->getRockerDown(),
             ghc->getKnob());
      data_1 = data;
    }
