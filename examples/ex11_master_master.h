...
class MasterMaster : public barrett::systems::SingleIO<
  ...
  public:
	explicit MasterMaster(barrett::systems::ExecutionManager *em,
	                      char *remoteHost, int port = 5555,
	                      const std::string &sysName = "MasterMaster")
	    : barrett::systems::SingleIO<jp_type, jp_type>(sysName), sock(-1),
	      linked(false), numMissed(NUM_MISSED_LIMIT), theirJp(0.0) {
		int err;
		long flags;
		int buflen;
		...
		if (sock == -1) {
			...
			throw std::runtime_error("(MasterMaster::MasterMaster): Ctor "
			                         "failed. Check /var/log/syslog.");
		}
		...
		syslog(LOG_ERR, "%s: Note, output buffer is %d bytes.", __func__,
		       buflen);
		...
		if (err == -1) {
			syslog(LOG_ERR, "%s: Could not bind to socket on port %d.",
			       __func__, port);
			throw std::runtime_error("(MasterMaster::MasterMaster): Ctor "
			                         "failed. Check /var/log/syslog.");
		}
		...
		if (err) {
			syslog(LOG_ERR, "%s: Could not set datagram destination.",
			       __func__);
			throw std::runtime_error("(MasterMaster::MasterMaster): Ctor "
			                         "failed. Check /var/log/syslog.");
		}
  ...
  protected:
	...
	virtual void operate() {

		{
			...
			if (numMissed <
			    NUM_MISSED_LIMIT) { // prevent numMissed from wrapping
				++numMissed;
			}
