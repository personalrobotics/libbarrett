...
template <size_t DOF>
class JpCircle
    : public systems::SingleIO<double,
                               typename units::JointPositions<DOF>::type> {
	BARRETT_UNITS_TEMPLATE_TYPEDEFS(DOF);

  public:
	JpCircle(jp_type startPos, double amplitude, double omega,
	         const std::string &sysName = "JpCircle")
	    : systems::SingleIO<double, jp_type>(sysName), jp(startPos), jp_0(jp),
	      amp(amplitude), omega(omega) {
...
class CpCircle
    : public systems::SingleIO<double, units::CartesianPosition::type> {
	BARRETT_UNITS_FIXED_SIZE_TYPEDEFS;

  public:
	CpCircle(cp_type startPos, double amplitude, double omega,
	         const std::string &sysName = "CpCircle")
	    : systems::SingleIO<double, cp_type>(sysName), cp(startPos), x_0(cp[0]),
	      y_0(cp[1]), amp(amplitude), omega(omega) {}
...
int wam_main(int argc, char **argv, ProductManager &pm,
	...
	if (DOF > 3) {
		...
		// Check configuration. Currently, there is no hardware support for
		// determining Proficio configuration. However, assuming that the user
		// has run leftConfig or rightConfig as needed, we can determine
		// configuration from the angle of joint 3. Joint 3 range is
		// [0.12, 2.76] for right-configured Proficio and [-2.76, -0.12] for
		// left-configured Proficio.
		if (temp[2] < 0) {
			// left configuration
		...
		std::cout << "Error: No known robot with DOF < 3. Quitting."
		          << std::endl;
		// error
		return -1;
	}
	...
	printf("Press [Enter] to move the end-point in circles using joint "
	       "position control.");
	waitForEnter();

	wam.moveTo(startPos);
	// Indicate the current position and the maximum rate limit to the rate
	// limiter
	...
	// Enforces that the individual joints move less than or equal to the above
	// mentioned rate limit
	...
	printf("Press [Enter] to move the end-point in circles using Cartesian "
	       "position control.");
	waitForEnter();
