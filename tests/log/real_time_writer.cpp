...
struct BigLogTraits {
	...
	static size_t serializedLength() { return 1025; }
...
void fillLogVerify(size_t n, size_t period) {
	...
	verifyFileContents(tmpFile, reinterpret_cast<char *>(ds),
	                   sizeof(double[n]));
...
TEST(RealTimeLogWriterTest, RecordRateCtorThrows) {
	...
	EXPECT_THROW(log::RealTimeWriter<double> lw2(tmpFile, 3.6e-6),
	             std::logic_error); // too fast

	std::remove(tmpFile);
}

TEST(RealTimeLogWriterTest, SmallSlow) { fillLogVerify(55, 500); }
...
TEST(RealTimeLogWriterTest, NormalSlow) { fillLogVerify(555, 500); }
