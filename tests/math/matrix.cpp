...
template <typename T> class FixedMatrixTypedTest : public ::testing::Test {
  public:
	FixedMatrixTypedTest() : a() {}

  protected:
	T a;
};

typedef ::testing::Types<math::Matrix<ROWS, COLS>, LocalUnits<ROWS, COLS>::type>
    FixedTypes;
TYPED_TEST_CASE(FixedMatrixTypedTest, FixedTypes);

// tests for dynamically sized matrices
template <typename T> class DynamicMatrixTypedTest : public ::testing::Test {
  public:
	DynamicMatrixTypedTest() : a(ROWS, COLS) {}

  protected:
	T a;
};

typedef ::testing::Types<math::Matrix<Eigen::Dynamic, Eigen::Dynamic>,
                         LocalUnits<Eigen::Dynamic, Eigen::Dynamic>::type>
    DynamicTypes;
TYPED_TEST_CASE(DynamicMatrixTypedTest, DynamicTypes);

// common tests
template <typename T> class MatrixTypedTest : public ::testing::Test {
  public:
	MatrixTypedTest() : a(ROWS, COLS) {}
...
TYPED_TEST(FixedMatrixTypedTest, ConfigCtorThrows) {
	...
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.two_one")),
	             std::runtime_error);
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.two_two")),
	             std::runtime_error);
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.two_three")),
	             std::runtime_error);

	EXPECT_THROW(TypeParam(config.lookup("matrix_test.three_one")),
	             std::runtime_error);
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.three_three")),
	             std::runtime_error);

	EXPECT_THROW(TypeParam(config.lookup("matrix_test.four_one")),
	             std::runtime_error);
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.four_two")),
	             std::runtime_error);
	EXPECT_THROW(TypeParam(config.lookup("matrix_test.four_three")),
	             std::runtime_error);
}
...
TYPED_TEST(MatrixTypedTest, CopyFromConfigThrows) {
	...
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.two_one")),
	             std::runtime_error);
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.two_two")),
	             std::runtime_error);
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.two_three")),
	             std::runtime_error);

	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.three_one")),
	             std::runtime_error);
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.three_three")),
	             std::runtime_error);

	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.four_one")),
	             std::runtime_error);
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.four_two")),
	             std::runtime_error);
	EXPECT_THROW(this->a.copyFrom(config.lookup("matrix_test.four_three")),
	             std::runtime_error);
}
...
TYPED_TEST(MatrixTypedTest, VectorVectorTraitsArithmetic) {
    ...
    TypeParam a1(ROWS,COLS), a2(ROWS,COLS), expected(ROWS,COLS),
result(ROWS,COLS);
