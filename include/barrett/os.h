...
/** btsleep and btsleepRT are a functions designed to allow pauses in operation.
 * btsleepRT is real-time safe.
 *
 */
...
/** highResolutionSystemTime maintains the current system time measured in
 * seconds. The resolution is 1 nanosecond when using Xenomai.
 */
 ...
 * boost::format object. http://www.boost.org/doc/libs/1_49_0/libs/format/ The
 * formatted message is output to syslog() and optionally stderr. Use the
 ...
 *   (barrett::logMessage("Bad parameter value: %.2f")
 * % 5.3277).raise<std::runtime_error>();
 */
