...
template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(double x, double y) : Base(x, y), gsl() {
	initGslType(&gsl);
}
...
template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(const double *data) : Base(data), gsl() {
	initGslType(&gsl);
}
...
// template<int R, int C, typename Units>
// template<typename OtherDerived>
// inline Matrix& Matrix<R,C, Units>::operator=(const
...
template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(double d) : Base(), gsl() {
	initGslType(&gsl);
	this->setConstant(d);
}

template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(int r, double d) : Base(r), gsl() {
	initGslType(&gsl);
	this->setConstant(d);
}

template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(int r, int c, double d) : Base(r, c), gsl() {
	initGslType(&gsl);
	this->setConstant(d);
}

template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(const gsl_type *gslType) : Base(), gsl() {
...
inline Matrix<R, C, Units>::Matrix(const libconfig::Setting &setting)
    : Base(), gsl() {
	resizeIfDynamic(setting.getLength(),
	                setting[0].isNumber() ? 1 : setting[0].getLength());
	initGslType(&gsl);
	copyFrom(setting);
}

template <int R, int C, typename Units>
inline Matrix<R, C, Units>::Matrix(const Matrix &a) : Base(a), gsl(a.gsl) {
	initGslType(&gsl);
}

template <int R, int C, typename Units> inline Matrix<R, C, Units>::~Matrix() {}
...
inline void Matrix<R, C, Units>::copyFrom(const libconfig::Setting &setting) {
	// special case: both column and row vectors can be initialized by a config
	// row vector
	if (this->isVector()) {
		if (setting.getLength() != this->size()) {
			...
			ss << "(math::Matrix<>::copyFrom(libconfig::Setting)): The size of "
			      ...
			      "Expected size "
			   << this->size() << ", got size " << setting.getLength()
	...
	if (setting.getLength() != this->rows()) {
		...
		ss << "(math::Matrix<>::copyFrom(libconfig::Setting)): Wrong number of "
		      "rows. Expected "
		   << this->rows() << ", got " << setting.getLength() << ". Path: \""
	...
	for (int i = 0; i < this->rows(); ++i) {
		if (setting[i].getLength() != this->cols()) {
			std::stringstream ss;
			ss << "(math::Matrix<>::copyFrom(libconfig::Setting)): Wrong "
			      "number of "
			      "columns. Expected "
			   << this->cols() << ", got " << setting[i].getLength()
			   << ". Path: \"" << setting[i].getPath()
...
template <int R, int C, typename Units>
inline typename Matrix<R, C, Units>::gsl_type *
Matrix<R, C, Units>::asGslType() {
	return &(this->gsl);
}

template <int R, int C, typename Units>
inline const typename Matrix<R, C, Units>::gsl_type *
Matrix<R, C, Units>::asGslType() const {
	return &(this->gsl);
}
...
void Matrix<R, C, Units>::checkSize(const gsl_matrix *g) const {
	if (g->size1 != static_cast<size_t>(this->rows()) ||
		...
		ss << "(math::Matrix<>::checkSize(gsl_matrix*)): The size of the "
		   ...
		   << this->rows() << "x" << this->cols() << ", got size " << g->size1
		   << "x" << g->size2;
		throw std::logic_error(ss.str());
	}
}
