...
template <typename T> class Spline {
  public:
	...
	Spline(const Container<tuple_type, Allocator> &samples,
	       bool saturateS = true);

	// initialDirection will be normalized internally
	template <template <typename, typename> class Container, typename Allocator>
	Spline(const Container<T, Allocator> &points,
	       /*const typename T::unitless_type& initialDirection = typename
	          T::unitless_type(0.0),*/
	       bool saturateS = true);
	...
	result_type operator()(double s) const { return eval(s); }
...
template <typename Scalar> class Spline<Eigen::Quaternion<Scalar>> {
  public:
	...
	template <template <typename, typename> class Container, typename Allocator>
	Spline(const Container<data_type, Allocator> &points,
	       bool saturateS = true);
...
class Spline<boost::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>> {
  public:
	...
	typedef detail::TupleSplineHolder<TUPLE_LEN, T0, T1, T2, T3, T4, T5, T6, T7,
	                                  T8, T9>
	    holder_type;
