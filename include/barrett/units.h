 ...
 * quantities. They are not interchangeable. They have different %units. A
 * user might want to design a joint-space position
 * controller and a joint-space velocity controller for the WAM. If the joint
 * position output of the systems::Wam were to be connected to the velocity
 * controller's feedback input, it would almost certainly result in a
 * programmer-caused error. If such a program were run, the robot would not
 * behave as intended and
 ...
 * template parameter
 * to math::Vector or math::Matrix. The type \c void is the default and lifts
 * all compile-time restrictions regarding %units.
 *
 * @see math::Vector
 * @see math::Matrix
