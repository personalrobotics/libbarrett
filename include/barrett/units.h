 ...
 *
 * Defines "unit types" that are actually specializations of math::Vector or
 * math::Matrix.
 *
 * These classes use type information to give meaning to what would otherwise be
 * an anonymous math::Vector of \c doubles.
 *
 * For instance, a systems::Wam has two outputs: one for joint positions and one
 * for joint velocities. Though both of these might be (depending on the
 * particular WAM) 7-element arrays of \c doubles, they represent two different
 * quantities. They are not interchangeable. They have different %units. A user
 * might want to design a joint-space position controller and a joint-space
 * velocity controller for the WAM. If the joint position output of the
 * systems::Wam were to be connected to the velocity controller's feedback
 * input, it would almost certainly result in a programmer-caused error. If such
 * a program were run, the robot would not behave as intended and finding the
 * bug could be difficult and time-consuming.
 *
 * Adding a notion of %units to our vectors (units::JointPositions::type,
 * units::JointVelocities::type, etc.) allows us to:
 *   - be explicit about our intents for any given input or parameter
 *   - more closely mimic the mathematical rules that govern our engineering
 * discipline
 *   - help the compiler to help us catch such errors.
 *   .
 * This comes without incurring a runtime performance penalty.
 *
 * Users can easily create their own barrett::units classes by passing any C++
 * type name (such as the name of a \c class or \c struct) as the \c Units
 * template parameter to math::Vector or math::Matrix. The type \c void is the
 * default and lifts all compile-time restrictions regarding %units.
 *
 * @see math::Vector
 * @see math::Matrix
...
/** Creates a standard set of \c typedefs in the local scope for the subset of
 * built-in barrett::units that have a fixed size.
 *
 * Many classes use this macro to define internal short-hand names for the
 * barrett::units they use frequently.
 *
 ...
 *   - \c pose_type  A boost::tuple<> type containing a cp_type and an
 * Eigen::Quaterniond
 *
 * @see BARRETT_UNITS_TYPEDEFS
 */
 ...
 * barrett::units they use frequently.
 *
 * \param dimension This positive integer is used in the \c typedefs below as
 * the template parameter for those types that require one.
 *
 * The following \c typedefs are defined:
 *   - \c sqm_type The unitless square barrett::math::Matrix of the given \c
 * dimension
 *   - \c v_type   The unitless barrett::math::Vector of the given \c dimension
 *   - \c jt_type  The barrett::units::JointTorques::type of the given \c
 * dimension
 *   - \c jp_type  The barrett::units::JointPositions::type of the given \c
 * dimension
 *   - \c jv_type  The barrett::units::JointVelocities::type of the given \c
 * dimension
 *   - \c ja_type  The barrett::units::JointAccelerations::type of the given \c
 * dimension
 ...
 * joint torques.\ Result available in the nested #type \c typedef.
...
/** Template metafunction yielding the R-element math::Vector used to represent
 * joint positions.\ Result available in the nested #type \c typedef.
 * @tparam R The number of rows in the resulting math::Vector
 */
...
/** Template metafunction yielding the R-element math::Vector used to represent
 * joint velocities.\ Result available in the nested #type \c typedef.
 * @tparam R The number of rows in the resulting math::Vector
 */
...
/** Template metafunction yielding the R-element math::Vector used to represent
 * joint accelerations.\ Result available in the nested #type \c typedef.
 * @tparam R The number of rows in the resulting math::Vector
 */
