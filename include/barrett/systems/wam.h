...
public:
   ...
   *
   *  Joint Torques are passed into the WAM, while providing the output joint
   * positions and joint vectors.
   */
public:
...
public:
   ...
   */
  ~Wam();
  /** trackReferenceSignal() used for following updating input. (any barrett
   * input units for control)
   */
  template <typename T>
  void trackReferenceSignal(
      System::Output<T>
          &referenceSignal); // NOLINT: non-const reference for syntax
  ...
  /** isGravityCompensated() returns flag as to the status of Gravity
   * Compensation
   */
  bool isGravityCompensated();
  /** moveHome(bool blocking, double velocity, double acceleration) method
   *sends WAM to stored Home Position
   *
   *	Blocking Allows/Disallows other functions to happen while Movement is
   *under way Velocity Speed value in m/s Acceleration Acceleration value in
   *m/s^2 Function is overloaded to allow multiple types of calls.
   */
   ...
   *
   *  Function is overloaded to allow moves any type of Barrett Unit Types
   *input. Destination Based on type cartesian(xyz), joint(radians),
   *quaternoind(orientation), pose blocking Determines whether program should
   *wait for move to finish before continuing velocity Speed at which to move
   *	acceleration value in radians per second
   */
  void moveTo(const jp_type &destination, bool blocking = true,
              double velocity = 0.5, double acceleration = 0.5);
  void moveTo(const cp_type &destination, bool blocking = true,
              double velocity = 0.1, double acceleration = 0.2);
  void moveTo(const Eigen::Quaterniond &destination, bool blocking = true,
              double velocity = 0.5, double acceleration = 0.5);
  void moveTo(const pose_type &destination, bool blocking = true,
              double velocity = 0.1, double acceleration = 0.2);
  template <typename T>
  void
  moveTo(const T &currentPos,
         /*const typename T::unitless_type& currentVel,*/ const T &destination,
         bool blocking, double velocity, double acceleration);
  /** moveIsDone() method returns false while the trajectory controller for
   * the most recent moveTo() command is still active.
   *
   ...
   *
   * To prevent uncontrolled falling, you should ensure you have set
   * gravityCompensate(true) before calling idle().
   */
  void idle();

  /** getEmMutex() method allows access to ExecutionManagers Mutex in
   * LowLevelWam Class.
   */
  ...
  const LowLevelWam<DOF> &getLowLevelWam() const {
    return llww.getLowLevelWam();
  }

protected:
  template <typename T> T currentPosHelper(const T &currentPos);
  template <typename T>
  void moveToThread(const T &currentPos, const T &destination, double velocity,
                    double acceleration, bool *started,
                    boost::shared_future<boost::thread *> threadPtrFuture);

  bool doneMoving;
  boost::thread_group mtThreadGroup;

  // Used to calculate TP and TO if the values aren't already being calculated
  // in the control loop.
