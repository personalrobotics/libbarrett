...
*/
  template <typename T>
  void trackReferenceSignal(System::Output<T> &referenceSignal); // NOLINT:
                                                                 // non-const
                                                                 // reference
                                                                 // for syntax
...
*/
  ...
  /** moveHome(bool blocking, double velocity, double acceleration) method sends
   *WAM to stored Home Position
   *
   ...
   *input.
   *	Destination Based on type cartesian(xyz), joint(radians),
   *quaternoind(orientation), pose
   *	blocking Determines whether program should wait for move to finish
   *before continuing
  ...
  /** moveIsDone() method returns false while the trajectory controller for the
   * most recent moveTo() command is still active.
   *
