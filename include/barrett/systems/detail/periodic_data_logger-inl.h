...
PeriodicDataLogger<T, LogWriterType>::PeriodicDataLogger(
    ExecutionManager *em, LogWriterType *logWriter, size_t periodMultiplier,
    const std::string &sysName)
    : System(sysName), SingleInput<T>(this), lw(logWriter), logging(true),
...
void PeriodicDataLogger<T, LogWriterType>::closeLog() {
  if (isLogging()) {
	...
	// lw::close() is probably not real-time safe, so keep it out of the
	// critical section.
	lw->close();
	delete lw;
	lw = NULL;
