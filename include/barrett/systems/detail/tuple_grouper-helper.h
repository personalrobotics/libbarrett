...
template <size_t N, typename T0, typename T1, typename T2, typename T3,
...
struct InputHolder
	...
	getInput() {
		...
		return (static_cast<InputHolder< // NOLINT: lint doesn't know that these
		                                 // are templates
		            Index + 1, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> *>(this))
		    ->input;
	}
	...
	System::Input<typename boost::tuples::element<N - 1, tuple_type>::type>
	    input;
...
struct InputHolder<1, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> {
	...
	explicit InputHolder(tg_type *parent) : input(parent), values() {}

	template <size_t InputIndex>
	System::Input<typename boost::tuples::element<InputIndex, tuple_type>::type>
	    &getInput() { // NOLINT: line length is for clarity
		return input;
	}

	bool valuesDefined() const { return input.valueDefined(); }
  ...
  protected:
	void collectValues() { boost::tuples::get<0>(values) = input.getValue(); }
};
