...
Summer<T, numInputs, RequiresAlignment>::Summer(
    const std::string &inputPolarity, bool undefinedIsZero,
    const std::string &sysName)
    : System(sysName), SingleOutput<T>(this), polarity(inputPolarity),
      strict(!undefinedIsZero), sum() {
	initInputs();
...
template <typename T, size_t numInputs, bool RequiresAlignment>
Summer<T, numInputs, RequiresAlignment>::Summer(const char *inputPolarity,
                                                bool undefinedIsZero,
                                                const std::string &sysName)
    : System(sysName), SingleOutput<T>(this), polarity(inputPolarity),
      strict(!undefinedIsZero), sum() {
...
template <typename T, size_t numInputs, bool RequiresAlignment>
Summer<T, numInputs, RequiresAlignment>::Summer(
    const std::bitset<numInputs> &inputPolarity, bool undefinedIsZero,
    const std::string &sysName)
    : System(sysName), SingleOutput<T>(this), polarity(inputPolarity),
      strict(!undefinedIsZero), sum() {
	initInputs();
...
template <typename T, size_t numInputs, bool RequiresAlignment>
Summer<T, numInputs, RequiresAlignment>::Summer(bool undefinedIsZero,
                                                const std::string &sysName)
