...
Wam<DOF>::Wam(ExecutionManager *em, const std::vector<Puck *> &genericPucks,
    ...
    : llww(em, genericPucks, safetyModule, setting["low_level"], torqueGroupIds,
           sysName + "::LowLevel"),
      kinematicsBase(setting["kinematics"]),
      gravity(setting["gravity_compensation"]),
      jvFilter(setting["joint_velocity_filter"]), toolPosition(),
      toolVelocity(), toolOrientation(), toolPose(),

      supervisoryController(), jtPassthrough(1.0),
      ...
      tpController(setting["tool_position_control"]), tf2jt(),
      toController(setting["tool_orientation_control"]), tt2jt(),

      tpoSplitter(), tpoTpController(setting["tool_position_control"]),
      tpoTf2jt(), tpoToController(setting["tool_orientation_control"]),
      tpoTt2jt(), tpoSum(),

      jtSum(true),

      input(jtSum.getInput(JT_INPUT)), jpOutput(llww.jpOutput),
...
void Wam<DOF>::moveToThread(
	...
	try {
		...
		*started = true;
		...
		// execution cycle. It may become undefined again if trajectory.output
		// is disconnected and this chain of Systems loses its ExecutionManager.
		// We must check that the value is defined before calling getValue().
