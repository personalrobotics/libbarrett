...
struct OutputHolder
  ...
  getOutput() {
    ...
    return (static_cast<OutputHolder< // NOLINT: lint doesn't know that
                                      // these are templates
...
struct OutputHolder<1, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> {
  ...
  explicit OutputHolder(ts_type *parent) : output(parent, &outputValue) {}

  template <size_t Index> System::Output<data_type> &getOutput() {
    return output;
  }
