...
inline void Puck::getProperty(enum Property prop,
                              typename Parser::result_type *result,
                              bool realtime) const {
	getProperty<Parser>(bus, id, getPropertyId(prop), result, realtime);
}

inline int Puck::getProperty(const bus::CommunicationsBus &bus, int id,
                             int propId, bool realtime) {
	int result;
...
template <typename Parser>
void Puck::getProperty(const bus::CommunicationsBus &bus, int id, int propId,
                       typename Parser::result_type *result, bool realtime) {
	int ret =
	    getPropertyHelper<Parser>(bus, id, propId, result, true, realtime, 0.0);
	if (ret != 0) {
...
inline int Puck::tryGetProperty(const bus::CommunicationsBus &bus, int id,
                                int propId, int *result, double timeout_s) {
	return tryGetProperty<StandardParser>(bus, id, propId, result, timeout_s);
}
template <typename Parser>
int Puck::tryGetProperty(const bus::CommunicationsBus &bus, int id, int propId,
                         typename Parser::result_type *result,
                         double timeout_s) {
	int ret = getPropertyHelper<Parser>(bus, id, propId, result, false, false,
	                                    timeout_s);
...
template <typename Parser>
int Puck::getPropertyHelper(const bus::CommunicationsBus &bus, int id,
                            int propId, typename Parser::result_type *result,
                            bool blocking, bool realtime, double timeout_s) {
	...
	return receiveGetPropertyReply<Parser>(bus, id, propId, result, blocking,
	                                       realtime);
}

inline void Puck::setProperty(const bus::CommunicationsBus &bus, int id,
                              int propId, int value, bool blocking) {
	...
	if (blocking) {
		// Make sure the command has completed so we don't overflow the Puck's
		// receive buffer.
		getProperty(bus, id, getPropertyId(STAT, PT_Unknown, 0));
	}
}

inline int Puck::sendGetPropertyRequest(const bus::CommunicationsBus &bus,
                                        int id, int propId) {
...
inline int Puck::receiveGetPropertyReply(const bus::CommunicationsBus &bus,
                                         int id, int propId, int *result,
                                         bool blocking, bool realtime) {
	return receiveGetPropertyReply<StandardParser>(bus, id, propId, result,
	                                               blocking, realtime);
}
template <typename Parser>
int Puck::receiveGetPropertyReply(const bus::CommunicationsBus &bus, int id,
                                  int propId,
                                  typename Parser::result_type *result,
                                  bool blocking, bool realtime) {
	...
	int ret =
	    bus.receive(Parser::busId(id, propId), data, len, blocking, realtime);
	if (ret != 0) {
		return ret;
	}

	// Make return code negative to avoid colliding with bus.receive() return
	// codes.
	return -Parser::parse(id, propId, result, data, len);
}

inline int Puck::getPropertyId(enum Property prop, enum PuckType pt,
                               int fwVers) {
	int propId = getPropertyIdNoThrow(prop, pt, fwVers);
	if (propId == -1) {
		(logMessage(
		     "Puck::%s(): Invalid property. "
		     "Pucks with type %s and firmware version %d do not respond to "
		     "property %s.") %
		 __func__ % getPuckTypeStr(pt) % fwVers % getPropertyStr(prop))
		    .raise<std::runtime_error>();
	}
	return propId;
}
