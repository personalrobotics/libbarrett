...
template <size_t DOF>
LowLevelWam<DOF>::LowLevelWam(const std::vector<Puck *> &_pucks,
                              SafetyModule *_safetyModule,
                              const libconfig::Setting &setting,
                              std::vector<int> torqueGroupIds)
    : MultiPuckProduct(DOF, _pucks, PuckGroup::BGRP_WAM, props,
                       sizeof(props) / sizeof(props[0]),
                       "LowLevelWam::LowLevelWam()"),
      safetyModule(_safetyModule), torqueGroups(), home(setting["home"]),
      j2mp(setting["j2mp"]), noJointEncoders(true),
      positionSensor(PS_MOTOR_ENCODER), lastUpdate(0.0),
      torquePropId(group.getPropertyId(Puck::T)) {
	logMessage("  Config setting: %s => \"%s\"") % setting.getSourceFile() %
	    setting.getPath();

	group.setProperty(Puck::MODE,
	                  MotorPuck::MODE_IDLE); // Make sure the Pucks are IDLE
	...
	if (!lu.isInvertible()) {
		(logMessage("LowLevelWam::%s(): j2mp matrix is not invertible.") %
		 __func__)
		    .template raise<std::runtime_error>();
	}
	m2jp = lu.inverse();
	j2mt = m2jp.transpose();
	...
	size_t numTorqueGroups =
	    ceil(static_cast<double>(DOF) / MotorPuck::PUCKS_PER_TORQUE_GROUP);
	...
	if (safetyModule == NULL) {
		...
		for (size_t i = 0; i < DOF; ++i) {
			currentAngle[i] =
			    motorPucks[i].counts2rad(pucks[i]->getProperty(Puck::MECH));
		}
		...
		for (size_t i = 0; i < DOF; ++i) {
			...
			if (pucks[i]->getVers() < 118) {
				logMessage("  No zero-compensation for Puck %d: old firmware") %
				    pucks[i]->getId();
				errorAngle[i] = 0;
				continue;
			}

			// If not ROLE & 256, then it's not an absolute encoder; don't
			// compensate
			if (!(pucks[i]->hasOption(Puck::RO_MagEncOnSerial))) {
				logMessage(
				    "  No zero-compensation for Puck %d: no absolute encoder") %
				    pucks[i]->getId();
				errorAngle[i] = 0;
				continue;
			}
			...
			if (zeroAngle[i] > 2 * M_PI || zeroAngle[i] < 0) {
				logMessage("  No zero-compensation for Puck %d: bad "
				           "calibration data") %
				    pucks[i]->getId();
				errorAngle[i] = 0;
				continue;
	...
	// If there are joint encoders, look up counts per revolution from the
	// config file
	if (numJe != 0) {
		noJointEncoders = false;
		logMessage("  Found %d joint encoders (%d are initialized)") % numJe %
		    numInitializedJe;
	...
	// Prevent velocity faults on startup due to WAM motion while no program was
	// running.
	if (safetyModule != NULL) {
...
template <size_t DOF> LowLevelWam<DOF>::~LowLevelWam() {
	detail::purge(torqueGroups);
}

template <size_t DOF>
inline const typename LowLevelWam<DOF>::jp_type &
LowLevelWam<DOF>::getJointPositions(enum PositionSensor sensor) const {
	switch (sensor) {
...
void LowLevelWam<DOF>::setPositionSensor(enum PositionSensor sensor) {
	...
	case PS_JOINT_ENCODER:
		if (!hasJointEncoders()) {
			(logMessage("LowLevelWam::%s: This WAM is not equipped with joint "
			            "encoders.") %
			 __func__)
			    .template raise<std::logic_error>();
		}
		...
		break;
	default:
		(logMessage("LowLevelWam::%s: Bad sensor value: %d") % __func__ %
		 sensor)
		    .template raise<std::logic_error>();
		break;
	}
...
template <size_t DOF> void LowLevelWam<DOF>::update() {
	double now = highResolutionSystemTime();

	if (noJointEncoders) {
		// Changing realtime to false. If this thread never yields, there is a
		// chance the CAN request will never go out.
		// group.getProperty<MotorPuck::MotorPositionParser<double> >(Puck::P,
		// pp.data(), true);
		group.getProperty<MotorPuck::MotorPositionParser<double>>(
		    Puck::P, pp.data(), false);
		jp_motorEncoder =
		    p2jp * pp; // Convert from Puck positions to joint positions
		jp_best = jp_motorEncoder;
	} else {
		// Make sure the reinterpret_cast below makes sense.
		BOOST_STATIC_ASSERT(
		    sizeof(MotorPuck::CombinedPositionParser<double>::result_type) ==
		    2 * sizeof(double));

		// PuckGroup::getProperty() will fill pp_jep.data() with 2*DOF doubles:
		// Primary Encoder 1, Secondary Encoder 1, Primary Encoder 2, Secondary
		// Encoder 2, ...
		group.getProperty<MotorPuck::CombinedPositionParser<double>>(
		    Puck::P,
		    reinterpret_cast<
		        MotorPuck::CombinedPositionParser<double>::result_type *>(
		        pp_jep.data()),
		    false);
...
template <size_t DOF> void LowLevelWam<DOF>::setTorques(const jt_type &jt) {
	// Get around C++ address-of-static-member weirdness...
	static const size_t PUCKS_PER_TORQUE_GROUP =
	    MotorPuck::PUCKS_PER_TORQUE_GROUP;
	...
	for (size_t g = 0; g < torqueGroups.size(); ++g) {
		MotorPuck::sendPackedTorques(bus, torqueGroups[g]->getId(),
		                             torquePropId, pt.data() + i,
		                             std::min(PUCKS_PER_TORQUE_GROUP, DOF - i));
		i += PUCKS_PER_TORQUE_GROUP;
	}
}

template <size_t DOF> void LowLevelWam<DOF>::definePosition(const jp_type &jp) {
