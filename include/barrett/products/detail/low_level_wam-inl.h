...
LowLevelWam<DOF>::LowLevelWam(const std::vector<Puck *> &_pucks,
  ...
  if (safetyModule == NULL) {
	...
	for (size_t i = 0; i < DOF; ++i) {
	  ...
	  if (zeroAngle[i] > 2 * M_PI || zeroAngle[i] < 0) {
		logMessage("  No zero-compensation for Puck %d: bad calibration data") %
		    pucks[i]->getId();
		errorAngle[i] = 0;
		continue;
	  }
  ...
  // If there are joint encoders, look up counts per revolution from the config
  // file
  if (numJe != 0) {
...
void LowLevelWam<DOF>::setPositionSensor(enum PositionSensor sensor) {
  ...
  case PS_JOINT_ENCODER:
	if (!hasJointEncoders()) {
	  (logMessage(
	       "LowLevelWam::%s: This WAM is not equipped with joint encoders.") %
	   __func__)
	      .template raise<std::logic_error>();
	}
	for (size_t i = 0; i < DOF; ++i) {
