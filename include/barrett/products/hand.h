...
class Hand : public MultiPuckProduct {
  public:
	...
	/** doneMoving Method */
	bool doneMoving(unsigned int whichDigits = WHOLE_HAND,
	                bool realtime = false) const;
	/** waitUntilDoneMoving Method */
	void waitUntilDoneMoving(unsigned int whichDigits = WHOLE_HAND,
	                         double period_s = 0.1) const;

	// Basic moves
	/** open Method */
	void open(unsigned int whichDigits = WHOLE_HAND,
	          bool blocking = true) const;
	/** open Method */
	void open(bool blocking) const { open(WHOLE_HAND, blocking); }
	/** close Method */
	void close(unsigned int whichDigits = WHOLE_HAND,
	           bool blocking = true) const;
	/** close Method */
	...
	void trapezoidalMove(const jp_type &jp,
	                     unsigned int whichDigits = WHOLE_HAND,
	                     bool blocking = true) const;
	/** trapezoidalMove Method */
	void trapezoidalMove(const jp_type &jp, bool blocking) const {
		trapezoidalMove(jp, WHOLE_HAND, blocking);
	}
	/** velocityMove Method */
	void velocityMove(const jv_type &jv,
	                  unsigned int whichDigits = WHOLE_HAND) const;
	...
	static const unsigned int S_ALL =
	    S_POSITION | S_FINGERTIP_TORQUE | S_TACT_FULL;
	/** update Method */
	...
	const std::vector<int> &getSecondaryEncoderPosition() const {
		return secondaryEncoder;
	}
	/** enableBreakawayEncoders Method actives or deactivates the Secondary
	 * Encoders */
	void enableBreakawayEncoders(bool enable) {
		useSecondaryEncoders = enable;
	} // Enabled by default.
	  /** hasFingertipTorqueSensors Method returns status of installed fingertip
	   * torque sensors */
	...
	/** hasTactSensors Method returns whether or not Tactile Sensors are present
	 * on hand. */
	bool hasTactSensors() const { return hasTact; }
	/** getTactilePucks Method creates container of pucks to get tactile sensor
	 * data from possible locations */
  ...
  protected:
	...
	bool digitsInclude(unsigned int whichDigits, size_t index) const {
		return whichDigits & (1 << index);
	}
	/** */
	void setProperty(unsigned int whichDigits, enum Puck::Property prop,
	                 int value) const;
	/** */
	void setProperty(unsigned int whichDigits, enum Puck::Property prop,
