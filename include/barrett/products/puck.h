...
public:
  ...
  static int getProperty(const bus::CommunicationsBus &bus, int id, int propId,
                         bool realtime = false);
  template <typename Parser>
  static void getProperty(const bus::CommunicationsBus &bus, int id, int propId,
                          typename Parser::result_type *result,
                          bool realtime = false);
  static int tryGetProperty(const bus::CommunicationsBus &bus, int id,
                            int propId, int *result, double timeout_s = 0.001);
  template <typename Parser>
  static int tryGetProperty(const bus::CommunicationsBus &bus, int id,
  ...
  static int sendGetPropertyRequest(const bus::CommunicationsBus &bus, int id,
                                    int propId);
  static int receiveGetPropertyReply(const bus::CommunicationsBus &bus, int id,
                                     int propId, int *result, bool blocking,
                                     bool realtime);
  template <typename Parser>
  static int receiveGetPropertyReply(const bus::CommunicationsBus &bus, int id,
                                     int propId,
...
private:
  template <typename Parser>
  static int getPropertyHelper(const bus::CommunicationsBus &bus, int id,
                               int propId, typename Parser::result_type *result,
