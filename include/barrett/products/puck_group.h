...
class PuckGroup {
  public:
	...
	void getProperty(enum Puck::Property prop, int results[],
	                 bool realtime = false) const;
	template <typename Parser>
	void getProperty(enum Puck::Property prop,
	                 typename Parser::result_type results[],
	                 bool realtime = false) const;
	...
	template <typename Parser>
	void receiveGetPropertyReply(int propId,
	                             typename Parser::result_type results[],
	                             bool realtime = false) const;
	...
	enum BroadcastGroup {
		...
		BGRP_LOWER_WAM =
		    Puck::GROUP_MASK | 1, // A packed-torque group (Pucks 1-4)
		BGRP_UPPER_WAM =
		    Puck::GROUP_MASK | 2, // A packed-torque group (Pucks 5-7)

		BGRP_HAND = Puck::GROUP_MASK | 5, // The whole hand (Pucks 11-14)
	};

	enum FeedbackGroup {
		// When responding to requests for a normal property, Pucks send to
	    // Group 6.
		FGRP_OTHER = Puck::GROUP_MASK | 6,

		// When responding to requests for motor encoder position, Pucks send to
		// Group 3.
		...
		// On BHands with tactile sensors, TOP10 formated TACT data is sent to
	    // Group
		// 8.
		FGRP_TACT_TOP10 = Puck::GROUP_MASK | 8,
		// On BHands with tactile sensors, FULL formated TACT data is sent to
	    // Group
		// 9.
