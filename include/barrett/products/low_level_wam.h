...
template <size_t DOF> class LowLevelWam : public MultiPuckProduct {
	BARRETT_UNITS_TEMPLATE_TYPEDEFS(DOF);

  public:
	// pucks must be ordered by joint and must break into torque groups as
	// arranged
	...
	const jp_type &
	getJointPositions(enum PositionSensor sensor = PS_BEST) const;
	...
	bool usingJointEncoder(size_t jointIndex) const {
		return useJointEncoder[jointIndex];
	}
