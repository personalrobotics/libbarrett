...
public:
  ...
  MotorPuck(Puck *puck = NULL)
      : SpecialPuck(Puck::PT_Motor), cts(0), rpc(0.0), cpr(0.0), ipnm(0) {
	setPuck(puck);
  }
  ~MotorPuck() {}
  ...
  int nm2i(double torque) const {
	return floor(math::saturate(torque * ipnm, MAX_PUCK_TORQUE));
  }

  bool foundIndexPulse() const {
	return (p->getProperty(Puck::ECMIN) != 0) ||
	       (p->getProperty(Puck::ECMAX) != 0);
  }

  static void sendPackedTorques(const bus::CommunicationsBus &bus, int groupId,
                                int propId, const double *pt, int numTorques);

  static const size_t PUCKS_PER_TORQUE_GROUP = 4;
  static const int MAX_PUCK_TORQUE = 8191;

  template <typename ResultType> struct MotorPositionParser {
  ...
  template <typename ResultType> struct SecondaryPositionParser {
	static int busId(int id, int propId) {
  ...
  template <typename ResultType> struct CombinedPositionParser {
	static int busId(int id, int propId) {
...
private:
  ...
  static ResultType twentyTwoBit2(unsigned char msb, unsigned char middle,
                                  unsigned char lsb);
};
}
