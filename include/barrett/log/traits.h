...
template <typename T> struct DefaultTraits {
  ...
  static size_t serializedLength() { return T::serializedLength(); }

  static void serialize(parameter_type source, char *dest) {
	source.serialize(dest);
  }

  static T unserialize(char *source) { return T::unserialize(source); }

  static void asCSV(parameter_type source, std::ostream &os) { os << source; }
};
...
template <typename T> struct PODTraits {
  ...
  static size_t serializedLength() { return sizeof(T); }

  static void serialize(parameter_type source, char *dest) {
	std::memcpy(dest, &source, serializedLength());
  }

  static T unserialize(char *source) { return *reinterpret_cast<T *>(source); }
...
template <>
struct Traits<unsigned long long> : public PODTraits<unsigned long long> {};

template <typename T, size_t N> struct Traits<::boost::array<T, N>> {
  typedef const ::boost::array<T, N> &parameter_type;

  static size_t serializedLength() { return N * Traits<T>::serializedLength(); }
...
template <typename Scalar> struct Traits<Eigen::Quaternion<Scalar>> {
  ...
  static size_t serializedLength() { return sizeof(Scalar) * 4; }
  ...
  static void asCSV(parameter_type source, std::ostream &os) {
	os << source.w() << "," << source.x() << "," << source.y() << ","
	   << source.z();
  }
};

// template<typename TraitsDerived> struct
// Traits<Eigen::MatrixBase<TraitsDerived> > :
//		public DefaultTraits<Eigen::MatrixBase<TraitsDerived> > {
//	typedef typename DefaultTraits<Eigen::MatrixBase<TraitsDerived>
...
struct Traits<math::Matrix<R, C, Units>>
    : public DefaultTraits<math::Matrix<R, C, Units>> {

  typedef typename DefaultTraits<math::Matrix<R, C, Units>>::parameter_type
      parameter_type;
...
struct Traits<boost::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>> {
  ...
  typedef detail::TupleTraitsHelper<NUM_INPUTS, Traits<tuple_type>>
      tuple_traits_helper;
