...
RealTimeWriter<T, Traits>::RealTimeWriter(const char *fileName,
	...
	if (this->recordLength > 1024) {
		throw(std::logic_error(
		    "(log::RealTimeWriter::RealTimeWriter()): This constructor was not "
		    "designed for records this big."));
	}
	...
	if (period < 0.003) {
		throw(std::logic_error(
		    "(log::RealTimeWriter::RealTimeWriter()): This constructor was not "
		    "designed for data rates this high."));
	}
	...
	if (currentPos >= endInBuff) {
		if (writeToDisk) {
			throw(std::overflow_error(
			    "(log::RealTimeWriter::putRecord()): The input buffer filled "
			    "up before the output buffer was written to disk."));
		}
