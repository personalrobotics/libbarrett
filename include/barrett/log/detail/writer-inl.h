...
Writer<T, Traits>::Writer(const char *fileName)
    : file(fileName, std::ios_base::binary),
      recordLength(Traits::serializedLength()) {
  if (recordLength == 0) {
...
template <typename T, typename Traits> Writer<T, Traits>::~Writer() {
  if (file.is_open()) {
	close();
  }
...
template <typename T, typename Traits> inline void Writer<T, Traits>::close() {
  file.close();
}
}
}
