...
Reader<T, Traits>::Reader(const char *fileName)
    : file(fileName, std::ios_base::binary),
      recordLength(Traits::serializedLength()), recordCount(0) {
  if (recordLength == 0) {
...
template <typename T, typename Traits> Reader<T, Traits>::~Reader() {
  if (file.is_open()) {
	close();
  }
...
template <typename T, typename Traits> inline T Reader<T, Traits>::getRecord() {
  file.read(buffer, recordLength);

  if (file.eof()) {
	throw(std::underflow_error("(log::Reader::getRecord()): The end of the "
	                           "file was reached. There are no more records to "
	                           "read."));
  }
...
template <typename T, typename Traits> inline void Reader<T, Traits>::close() {
  file.close();
}
}
}
