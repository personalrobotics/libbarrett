...
PeriodicLoopTimer::PeriodicLoopTimer(double period_, int threadPriority)
  ...
  if (ret != 0 && ret != -EBUSY) {
    (logMessage("PeriodicLoopTimer::%s: rt_task_shadow(): (%d) %s") % __func__ %
     -ret % strerror(-ret))
        .raise<std::runtime_error>();
  }
...
/*
        ...
        const double remainder = releasePoint - now; 	// Calculate the amount
   of time remaining until our next release point if (remainder <= 0) {
   // If we are already past our next scheduled release point, then we missed it
   releasePoint = now + period; 				// Schedule a
   new release point at period seconds into the future

            if (firstRun) {
                firstRun = false;
                return 0;
   // Don't worry if we missed the very first release point } else {
