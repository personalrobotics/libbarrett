...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(Puck_setProperty_overloads, setProperty,
                                       2, 3)
...
tuple staticTryGetProperty(const bus::CommunicationsBus &bus, int id,
                           int propId, double timeout_s = -1.0) {
  int ret;
  int result = 0;
...
BOOST_PYTHON_FUNCTION_OVERLOADS(staticTryGetProperty_overloads,
                                staticTryGetProperty, 3, 4)

BOOST_PYTHON_FUNCTION_OVERLOADS(staticSetProperty_overloads, Puck::setProperty,
                                4, 5)

tuple receiveGetPropertyReply(const bus::CommunicationsBus &bus, int id,
                              int propId, bool blocking) {
  int ret;
  int result = 0;
  ret =
      Puck::receiveGetPropertyReply(bus, id, propId, &result, blocking, false);
  return make_tuple(ret, result);
}
...
void pythonProductsPuckInterface() {
  ...
  scope puckScope =
      class_<Puck>("Puck", init<const bus::CommunicationsBus &,
                                int>()[with_custodian_and_ward<1, 2>()])
          .def_readonly("NUM_PROPERTIES", NUM_PROPERTIES)
          .def("getPuckTypeStr", &Puck::getPuckTypeStr)
          .staticmethod("getPuckTypeStr")
          .def("getPropertyStr", &Puck::getPropertyStr)
          .staticmethod("getPropertyStr")
          .def("getPropertyEnum", &Puck::getPropertyEnum)
          .staticmethod("getPropertyEnum")
          .def("getPropertyEnumNoThrow", &Puck::getPropertyEnumNoThrow)
          .staticmethod("getPropertyEnumNoThrow")

          .def("wake",
               (void(Puck::*)()) & Puck::wake) // Cast to resolve the overload
          // TODO(dc): Expose getProperty() with alternate parsers?
          .def("getProperty", &getProperty)
          .def("setProperty",
               (void(Puck::*)(enum Puck::Property, int, bool) const) &
                   Puck::setProperty,
          ...
          .def("respondsToProperty",
               (bool(Puck::*)(enum Puck::Property) const) &
                   Puck::respondsToProperty)
          .def("getPropertyId",
               (int(Puck::*)(enum Puck::Property) const) & Puck::getPropertyId)
          .def("getPropertyIdNoThrow",
               (int(Puck::*)(enum Puck::Property) const) &
                   Puck::getPropertyIdNoThrow)
          ...
          .def("wakeList", &wakeList)
          .staticmethod("wakeList") // Renamed because boost::puthon can't
                                    // handle a static/non-static overload
      ;
  ...
  class_<Namespace, boost::noncopyable>("LowLevel", no_init)
      ...
      .def("getProperty", &staticGetProperty)
      .staticmethod("getProperty")
      .def("tryGetProperty", &staticTryGetProperty,
           staticTryGetProperty_overloads())
      .staticmethod("tryGetProperty")
      .def("setProperty",
           (void (*)(const bus::CommunicationsBus &, int, int, int, bool)) &
               Puck::setProperty,
           staticSetProperty_overloads())
      .staticmethod("setProperty")

      .def("sendGetPropertyRequest", &Puck::sendGetPropertyRequest)
      .staticmethod("sendGetPropertyRequest")
      .def("receiveGetPropertyReply", &receiveGetPropertyReply)
      .staticmethod("receiveGetPropertyReply")

      .def("respondsToProperty",
           (bool (*)(enum Puck::Property, enum Puck::PuckType, int)) &
               Puck::respondsToProperty)
      .staticmethod("respondsToProperty")
      .def("getPropertyId",
           (int (*)(enum Puck::Property, enum Puck::PuckType, int)) &
               Puck::getPropertyId)
      .staticmethod("getPropertyId")
      .def("getPropertyIdNoThrow",
           (int (*)(enum Puck::Property, enum Puck::PuckType, int)) &
               Puck::getPropertyIdNoThrow)
      .staticmethod("getPropertyIdNoThrow")
      ...
      .def("nodeId2BusId", &Puck::nodeId2BusId)
      .staticmethod("nodeId2BusId")
      .def("busId2NodeId", &Puck::busId2NodeId)
      .staticmethod("busId2NodeId")
      .def("encodeBusId", &Puck::encodeBusId)
      .staticmethod("encodeBusId")
      .def("decodeBusId", &decodeBusId)
      .staticmethod("decodeBusId")
