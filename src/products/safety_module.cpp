...
SafetyModule::SafetyModule(Puck *puck) : SpecialPuck(Puck::PT_Safety) {
	setPuck(puck);
...
void SafetyModule::waitForMode(enum SafetyMode mode, bool printMessage,
                               double pollingPeriod_s) {
	if (getMode() == mode) {
		return;
	}
...
enum SafetyModule::SafetyMode
SafetyModule::waitForModeChange(double pollingPeriod_s) {
...
void SafetyModule::ignoreNextVelocityFault() {
	p->setProperty(Puck::IFAULT,
	               SafetyModule::VELOCITY_FAULT_HISTORY_BUFFER_SIZE);
}
...
void SafetyModule::getPendantState(PendantState *ps, bool realtime) const {
	...
	for (int i = 0; i < PendantState::NUM_PARAMS; ++i) {
		...
		if (paramBits.count() != 1) { // exactly one of which should be set.
			(logMessage(
			     "SafetyModule::%s(): Bad PEN value for Parameter %d: %s") %
			 __func__ % i % bits.to_string())
			    .raise<std::runtime_error>();
		}
...
const char SafetyModule::safetyModeStrs[][15] = {"E-stop", "Shift-idle",
                                                 "Shift-activate"};

bool SafetyModule::PendantState::allSafe() const {
	return std::count(safetyParameters, safetyParameters + NUM_PARAMS, SAFE) ==
	       NUM_PARAMS;
}

bool SafetyModule::PendantState::hasFaults() const {
	return std::find(safetyParameters, safetyParameters + NUM_PARAMS, FAULT) !=
	       safetyParameters + NUM_PARAMS;
}
