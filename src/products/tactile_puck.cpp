...
int TactilePuck::FullTactParser::parse(int id, int propId, result_type *result,
  ...
  if (len != 8) {
    logMessage("%s: expected message length of 8, got message length of %d") %
        __func__ % len;
    return 1;
  }
  ...
  (*result)[i++] = ((((int)data[0] & 0x000F) << 8) | ((int)data[1] & 0x00FF)) /
                   FULL_SCALE_FACTOR;
  (*result)[i++] =
      ((((int)data[2] & 0x00FF) << 4) | (((int)data[3] & 0x00F0) >> 4)) /
      FULL_SCALE_FACTOR;
  (*result)[i++] = ((((int)data[3] & 0x000F) << 8) | ((int)data[4] & 0x00FF)) /
                   FULL_SCALE_FACTOR;
  (*result)[i++] =
      ((((int)data[5] & 0x00FF) << 4) | (((int)data[6] & 0x00F0) >> 4)) /
      FULL_SCALE_FACTOR;
  if (i < NUM_SENSORS) {
    (*result)[i] = ((((int)data[6] & 0x000F) << 8) | ((int)data[7] & 0x00FF)) /
                   FULL_SCALE_FACTOR;
  }
