...
void CANSocket::open(int port) {
  ...
  if (ret != 0) {
    close();
    (logMessage("CANSocket::%s(): Could not open CAN port. "
                "rt_dev_ioctl(SIOCGCANSTATE): (%d) %s") %
     __func__ % -ret % strerror(-ret))
        .raise<std::runtime_error>();
  } else {
    ...
    if (*state != CAN_STATE_ACTIVE) {
      ...
      const char canStateStrs[NUM_STATES][18] = {
          "active",  "warning",  "passive",      "bus-off", "scanning-baudrate",
          "stopped", "sleeping", "unknown state"};

      int index = *state;
      if (index < 0 || index >= NUM_STATES) {
        index = NUM_STATES - 1;
      }
      logMessage("  WARNING: CAN_STATE is %s (%d)") % canStateStrs[index] %
          *state;
  ...
  ret = rt_dev_setsockopt(handle->h, SOL_CAN_RAW, CAN_RAW_FILTER, &recvFilter,
                          sizeof(recvFilter));
  if (ret != 0) {
    close();
    (logMessage("CANSocket::%s(): Could not open CAN port. "
                "rt_dev_setsockopt(CAN_RAW_FILTER): (%d) %s") %
     __func__ % -ret % strerror(-ret))
        .raise<std::runtime_error>();
  }
  ...
  if (ret != 0) {
    close();
    (logMessage("CANSocket::%s(): Could not open CAN port. rt_dev_bind(): "
                "(%d) %s") %
     __func__ % -ret % strerror(-ret))
        .raise<std::runtime_error>();
  }
  ...
  if (ret < 0) {
    ...
    case -EAGAIN: // -EWOULDBLOCK
      logMessage("CANSocket::%s: "
                 "rt_dev_send(): data would block during non-blocking "
                 "send (output buffer full)") %
          __func__;
      return 1;
      break;
    case -ETIMEDOUT:
...
int CANSocket::receiveRaw(int &busId, unsigned char *data, size_t &len,
  ...
  struct can_frame frame;
  int ret = rt_dev_recv(handle->h, (void *)&frame, sizeof(can_frame_t),
