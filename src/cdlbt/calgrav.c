...
int bt_calgrav_create(struct bt_calgrav **gravptr, config_setting_t *gravconfig,
  ...
  if (!(mus = config_setting_get_member(gravconfig, "mus")) ||
    ...
    syslog(LOG_ERR, "%s: grav:mus not a list with %u elements.", __func__,
           (unsigned int)dof);
    bt_calgrav_destroy(grav);
    return -1;
  }
  ...
  for (j = 0; j < dof; j++) {
    ...
    /* Do memory checking */
    if (!grav->g[j] || !grav->mu[j] || !grav->t[j] || !grav->pt[j]) {
      syslog(LOG_ERR, "%s: Out of memory.", __func__);
      bt_calgrav_destroy(grav);
      return -1;
    }
...
int bt_calgrav_eval(struct bt_calgrav *grav, struct bt_kinematics *kin,
  ...
  for (j = grav->dof - 1; j >= 0; j--) {
    ...
    gsl_blas_dgemv(CblasTrans, 1.0, link->rot_to_world, grav->world_g, 0.0,
                   grav->g[j]);
    ...
    if (j < grav->dof - 1)
      gsl_blas_dgemv(CblasNoTrans, 1.0, link->next->rot_to_prev, grav->t[j + 1],
                     1.0, grav->t[j]);
    /* Move torque vector to the previous frame */
    gsl_blas_dgemv(CblasNoTrans, 1.0, link->rot_to_prev, grav->t[j], 0.0,
                   grav->pt[j]);
