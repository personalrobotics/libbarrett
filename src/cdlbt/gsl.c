...
int bt_gsl_fill_vector_cfgarray(gsl_vector *vec, config_setting_t *array) {
  ...
  if (config_setting_type(array) != CONFIG_TYPE_ARRAY &&
      config_setting_type(array) != CONFIG_TYPE_LIST)
    return -1;
  if (config_setting_length(array) != vec->size)
    return -1;
  for (i = 0; i < vec->size; i++) {
...
int bt_gsl_fill_matrix(gsl_matrix *mat, config_setting_t *parent,
  ...
  if (rows == NULL)
    return -1;
  if (config_setting_type(rows) != CONFIG_TYPE_LIST)
    return -1;
  if (config_setting_length(rows) != mat->size1)
    return -1;
  for (i = 0; i < mat->size1; i++) {
    config_setting_t *row;
    row = config_setting_get_elem(rows, i);
    if (config_setting_type(row) != CONFIG_TYPE_ARRAY &&
        config_setting_type(row) != CONFIG_TYPE_LIST)
      return -1;
    if (config_setting_length(row) != mat->size2)
      return -1;
