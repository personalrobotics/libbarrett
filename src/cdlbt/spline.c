...
int bt_spline_add(struct bt_spline *spline, const gsl_vector *vec, double s) {
	...
	for (i = 0; i < spline->dimension; i++) {
		spline->points[i] = (double *)realloc(
		    spline->points[i], (spline->npoints + 1) * sizeof(double));
	...
	if (spline->mode == BT_SPLINE_MODE_EXTERNAL) {
		spline->ss = (double *)realloc(spline->ss,
		                               (spline->npoints + 1) * sizeof(double));
...
int bt_spline_init(struct bt_spline *spline, gsl_vector *start,
	...
	if (!spline->interps) {
		spline->interps =
		    (gsl_interp **)malloc((spline->dimension) * sizeof(gsl_interp *));
		for (i = 0; i < spline->dimension; i++)
			spline->interps[i] = 0;
	}
	for (i = 0; i < spline->dimension; i++) {
		if (spline->interps[i])
			gsl_interp_free(spline->interps[i]);
		spline->interps[i] = gsl_interp_alloc(bt_interp, spline->npoints);

		if (direction) {
			bt_interp_set_type(spline->interps[i], BT_INTERP_SLOPE,
			                   BT_INTERP_NATURAL);
			bt_interp_set_slopes(spline->interps[i],
			                     gsl_vector_get(direction, i), 0.0);
		} else
			bt_interp_set_type(spline->interps[i], BT_INTERP_NATURAL,
			                   BT_INTERP_NATURAL);

		gsl_interp_init(spline->interps[i], spline->ss, spline->points[i],
		                spline->npoints);
	}
...
int bt_spline_get(struct bt_spline *spline, gsl_vector *result, double s) {
	...
	for (i = 0; i < spline->dimension; i++) {
		double coord;
		coord = gsl_interp_eval(spline->interps[i], spline->ss,
		                        spline->points[i], s, spline->acc);
