...
int bt_kinematics_create(struct bt_kinematics **kinptr,
  ...
  for (i = 0; i < kin->nlinks; i++) {
    ...
    link =
        (struct bt_kinematics_link *)malloc(sizeof(struct bt_kinematics_link));
    if (!link) {
      syslog(LOG_ERR, "%s: Out of memory.", __func__);
      bt_kinematics_destroy(kin);
      return -1;
  ...
  for (i = 0; i < kin->nlinks; i++) {
    ...
    if (link == kin->base) {
      ...
      if ((world = config_setting_get_member(kinconfig, "world_to_base"))) {
        ...
        for (j = 0; j < 4; ++j) {
          ...
          for (k = 0; k < 4; ++k) {
            ...
            if (bt_gsl_config_get_double(config_setting_get_elem(world_row, k),
                                         &val)) {
      ...
      if ((err = bt_gsl_config_double_from_group(toolplate_grp, "alpha_pi",
                                                 &(link->alpha))) ||
          (err = bt_gsl_config_double_from_group(toolplate_grp, "theta_pi",
                                                 &(link->theta))) ||
...
int bt_kinematics_eval(struct bt_kinematics *kin, const gsl_vector *jposition,
  ...
  if (jvelocity) {
    gsl_blas_dgemv(CblasNoTrans, 1.0, kin->tool_jacobian_linear, jvelocity, 0.0,
                   kin->tool_velocity);
