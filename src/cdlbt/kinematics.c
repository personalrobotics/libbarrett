...
int bt_kinematics_create(struct bt_kinematics **kinptr,
	...
	kin->link_array = (struct bt_kinematics_link **)malloc(
	    kin->nlinks * sizeof(struct bt_kinematics_link *));
	if (!kin->link_array) {
		syslog(LOG_ERR, "%s: Out of memory.", __func__);
	...
	for (i = 0; i < kin->nlinks; i++) {
		...
		link = (struct bt_kinematics_link *)malloc(
		    sizeof(struct bt_kinematics_link));
		if (!link) {
			syslog(LOG_ERR, "%s: Out of memory.", __func__);
			bt_kinematics_destroy(kin);
			return -1;
	...
	if (!(moving = config_setting_get_member(kinconfig, "moving")) ||
		...
		syslog(LOG_ERR, "%s: kin:moving not a list with %d elements.", __func__,
		       ndofs);
		bt_kinematics_destroy(kin);
		return -1;
	}
	...
	for (i = 0; i < kin->nlinks; i++) {
		...
		if (link == kin->base) {
			...
			/* Make sure we have an appropriate configuration for the world to
			 * base transform */
			if ((world =
			         config_setting_get_member(kinconfig, "world_to_base"))) {
				if ((config_setting_type(world) != CONFIG_TYPE_LIST) ||
				    (config_setting_length(world) != 4)) {
					syslog(LOG_ERR,
					       "%s: kinematics:world_to_base not a list with 4 "
					       "elements.",
					       __func__);
					bt_kinematics_destroy(kin);
					return -1;
				...
				for (j = 0; j < 4; ++j) {
					...
					if ((config_setting_type(world_row) != CONFIG_TYPE_LIST) ||
						...
						syslog(LOG_ERR,
						       "%s: kinematics:world_to_base #%d not a "
						       "4-element list.",
						       __func__, j);
						bt_kinematics_destroy(kin);
						return -1;
					...
					for (k = 0; k < 4; ++k) {
						...
						if (bt_gsl_config_get_double(
						        config_setting_get_elem(world_row, k), &val)) {
							syslog(LOG_ERR, "%s: that's not a number!",
							       __func__);
							bt_kinematics_destroy(kin);
						}
				...
				/* If unspecified, set the base-to-world transform to the
				 * identity by default */
			...
			if (!(toolplate_grp =
			          config_setting_get_member(kinconfig, "toolplate")) ||
			...
			if ((err = bt_gsl_config_double_from_group(
			         toolplate_grp, "alpha_pi", &(link->alpha))) ||
			    (err = bt_gsl_config_double_from_group(
			         toolplate_grp, "theta_pi", &(link->theta))) ||
			    (err = bt_gsl_config_double_from_group(toolplate_grp, "a",
			                                           &(link->a))) ||
			    (err = bt_gsl_config_double_from_group(toolplate_grp, "d",
			                                           &(link->d)))) {
				syslog(LOG_ERR,
				       "%s: No alpha_pi, theta_pi, a, and/or d in toolplate, "
				       "or not a number.",
				       __func__);
				bt_kinematics_destroy(kin);
				return -1;
			...
			/* Grab each of the dh parameters */
			if ((err = bt_gsl_config_double_from_group(link_grp, "alpha_pi",
			                                           &(link->alpha))) ||
			    (err = bt_gsl_config_double_from_group(link_grp, "a",
			                                           &(link->a))) ||
			    (err = bt_gsl_config_double_from_group(link_grp, "d",
			                                           &(link->d)))) {
				syslog(LOG_ERR,
				       "%s: No alpha_pi, theta_pi, a, and/or d in link %d, or "
				       "not a number.",
				       __func__, j);
				bt_kinematics_destroy(kin);
				return -1;
			...
			/* Initialize the matrices (the last two rows are const for
			 * revolute) */
			link->trans_to_prev = gsl_matrix_calloc(4, 4);
			if (!link->trans_to_prev) {
				syslog(LOG_ERR, "%s: Out of memory.", __func__);
...
int bt_kinematics_eval(struct bt_kinematics *kin, const gsl_vector *jposition,
	...
	bt_kinematics_eval_jacobian(kin, kin->dof, kin->tool->origin_pos,
	                            kin->tool_jacobian);
	/* Calculate the tool Cartesian velocity */
	if (jvelocity) {
		gsl_blas_dgemv(CblasNoTrans, 1.0, kin->tool_jacobian_linear, jvelocity,
		               0.0, kin->tool_velocity);
		/* Calculate the tool Cartesian angular velocity */
		gsl_blas_dgemv(CblasNoTrans, 1.0, kin->tool_jacobian_angular, jvelocity,
...
static int eval_trans_to_world(struct bt_kinematics_link *link) {
	/* Matrix Multiply */
	gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, link->prev->trans_to_world,
	               link->trans_to_prev, 0.0, link->trans_to_world);
	return 0;
}
