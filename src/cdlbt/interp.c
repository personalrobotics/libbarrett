  ...
  default: {
    ...
    for (i = 0; i < sys_size; i++) {
      ...
      if (state->ltype == BT_INTERP_SLOPE && i == 0) {
        ...
        state->cstate->g[i] =
            3.0 *
            (ydiff_ip1 * g_ip1 - 0.5 * (3.0 * (ydiff_i * g_i) - state->alpha));
        continue;
      }
      if (state->rtype == BT_INTERP_SLOPE && i == sys_size - 1) {
        ...
        state->cstate->g[i] =
            3.0 *
            (0.5 * (3.0 * (ydiff_ip1 * g_ip1) - state->beta) - ydiff_i * g_i);
        continue;
      }
      /* ####################### */
    ...
    /* Solve the system for the inner c's */
    {
      ...
      int status =
          gsl_linalg_solve_symm_tridiag(&diag_vec.vector, &offdiag_vec.vector,
                                        &g_vec.vector, &solution_vec.vector);
...
static int eval(const void *vstate, const double x_array[],
  ...
  case 3:
    if (x < x_array[1]) {
      double delx = x - x_array[0];
      *result = state->s_3p->aL +
                delx * (state->s_3p->bL +
                        delx * (state->s_3p->cL + delx * state->s_3p->dL));
      return GSL_SUCCESS;
    } else {
      double delx = x - x_array[1];
      *result = state->s_3p->aR +
                delx * (state->s_3p->bR +
                        delx * (state->s_3p->cR + delx * state->s_3p->dR));
      return GSL_SUCCESS;
    }
  default:
    return gsl_interp_cspline->eval(state->cstate, x_array, y_array, size, x,
                                    acc, result);
  }
}
...
static int eval_deriv(const void *vstate, const double x_array[],
  ...
  default:
    return gsl_interp_cspline->eval_deriv(state->cstate, x_array, y_array, size,
                                          x, acc, result);
  }
}
...
static int eval_integ(const void *vstate, const double x_array[],
  ...
  case 3:
    GSL_ERROR("wam_interp: integrating with size=2,3 not yet implemented.",
              GSL_EINVAL);
  default:
    return gsl_interp_cspline->eval_integ(state->cstate, x_array, y_array, size,
                                          acc, a, b, result);
  }
}
