...
static int init(void *vstate, const double xa[], const double ya[],
                size_t size) {
	...
	case 2: {
		...
		/* Solve the system */
		{
			...
			gsl_linalg_LU_solve(&A_view.matrix, p, &b_view.vector,
			                    &x_view.vector);
	...
	default: {
		...
		size_t sys_size =
		    max_index - 1; /* linear system is sys_size x sys_size */
		...
		{
			gsl_vector_view g_vec =
			    gsl_vector_view_array(state->cstate->g, sys_size);
			gsl_vector_view diag_vec =
			    gsl_vector_view_array(state->cstate->diag, sys_size);
			gsl_vector_view offdiag_vec =
			    gsl_vector_view_array(state->cstate->offdiag, sys_size - 1);
			gsl_vector_view solution_vec =
			    gsl_vector_view_array((state->cstate->c) + 1, sys_size);

			int status = gsl_linalg_solve_symm_tridiag(
			    &diag_vec.vector, &offdiag_vec.vector, &g_vec.vector,
			    &solution_vec.vector);
		...
		if (state->ltype == BT_INTERP_SLOPE) {
			...
			state->cstate->c[0] = 1.0 / (2.0 * h_0) *
			                      ((3.0 / h_0) * ydiff_0 - 3.0 * state->alpha -
			                       state->cstate->c[1] * h_0);
		} else
		...
		if (state->rtype == BT_INTERP_SLOPE) {
			...
			state->cstate->c[max_index] =
			    -1.0 / (2.0 * h_nm1) *
			    ((3.0 / h_nm1) * ydiff_nm1 - 3.0 * state->beta +
			     state->cstate->c[max_index - 1] * h_nm1);
		} else
...
static int eval(const void *vstate, const double x_array[],
	...
	case 2: {
		...
		*result = state->s_2p->a +
		          delx * (state->s_2p->b +
		                  delx * (state->s_2p->c + delx * state->s_2p->d));
		return GSL_SUCCESS;
	}
	case 3:
		if (x < x_array[1]) {
			...
			*result =
			    state->s_3p->aL +
			    delx * (state->s_3p->bL +
			            delx * (state->s_3p->cL + delx * state->s_3p->dL));
			return GSL_SUCCESS;
		} else {
			...
			*result =
			    state->s_3p->aR +
			    delx * (state->s_3p->bR +
			            delx * (state->s_3p->cR + delx * state->s_3p->dR));
			return GSL_SUCCESS;
		}
	default:
		return gsl_interp_cspline->eval(state->cstate, x_array, y_array, size,
		                                x, acc, result);
	}
}

static int eval_deriv(const void *vstate, const double x_array[],
                      const double y_array[], size_t size, double x,
                      gsl_interp_accel *acc, double *result) {
	struct state_t *state = (struct state_t *)vstate;
	switch (size) {
	case 2: {
		double delx = x - x_array[0];
		*result = state->s_2p->b +
		          delx * (2 * state->s_2p->c + delx * 3 * state->s_2p->d);
		return GSL_SUCCESS;
	}
	case 3:
		if (x < x_array[1]) {
			double delx = x - x_array[0];
			*result = state->s_3p->bL +
			          delx * (2 * state->s_3p->cL + delx * 3 * state->s_3p->dL);
			return GSL_SUCCESS;
		} else {
			double delx = x - x_array[1];
			*result = state->s_3p->bR +
			          delx * (2 * state->s_3p->cR + delx * 3 * state->s_3p->dR);
			return GSL_SUCCESS;
		}
	default:
		return gsl_interp_cspline->eval_deriv(state->cstate, x_array, y_array,
		                                      size, x, acc, result);
	}
}

static int eval_deriv2(const void *vstate, const double x_array[],
                       const double y_array[], size_t size, double x,
                       gsl_interp_accel *acc, double *result) {
	struct state_t *state = (struct state_t *)vstate;
	switch (size) {
	...
	default:
		return gsl_interp_cspline->eval_deriv2(state->cstate, x_array, y_array,
		                                       size, x, acc, result);
	}
}

static int eval_integ(const void *vstate, const double x_array[],
	...
	case 2:
	case 3:
		GSL_ERROR("wam_interp: integrating with size=2,3 not yet implemented.",
		          GSL_EINVAL);
	default:
		return gsl_interp_cspline->eval_integ(state->cstate, x_array, y_array,
		                                      size, acc, a, b, result);
	}
}
...
static void w_free(void *vstate) {
	...
	if (state->s_3p)
		free(state->s_3p);
	if (state->cstate)
		gsl_interp_cspline->free(state->cstate);
...
static const gsl_interp_type bt_interp_type = {
    "wam-interp", 2,           &alloc, &init, &eval, &eval_deriv,
    &eval_deriv2, &eval_integ, &w_free};
