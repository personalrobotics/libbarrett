...
int bt_profile_create(struct bt_profile **profileptr, double vel, double acc,
  ...
  v_diff = vel - v_init;
  if (length <
      (0.5 * vel * vel + 0.5 * v_diff * v_diff + v_init * v_diff) / acc) {
    double v_top;
    v_top = sqrt(length * acc + 0.5 * v_init * v_init);

    profile->time_endup = (v_top - v_init) / acc;
    profile->s_endup = v_init * profile->time_endup +
                       0.5 * acc * profile->time_endup * profile->time_endup;
    ...
    profile->s_end = profile->s_startdown +
                     0.5 * v_top * v_top / acc; /* Let's home this is length! */
  ...
  profile->time_startdown =
      profile->time_endup + (profile->s_startdown - profile->s_endup) / vel;
