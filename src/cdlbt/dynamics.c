...
int bt_dynamics_create(struct bt_dynamics **dynptr, config_setting_t *dynconfig,
  ...
  for (i = 0; i < dyn->nlinks; i++) {
    struct bt_dynamics_link *link;
    link = (struct bt_dynamics_link *)malloc(sizeof(struct bt_dynamics_link));
    if (!link) {
      syslog(LOG_ERR, "%s: Out of memory.", __func__);
      bt_dynamics_destroy(dyn);
      return -1;
  ...
  for (i = 0; i < dyn->nlinks; i++) {
    ...
    if (link == dyn->base) {
      ...
      /* Do memory checks */
      if (!link->com || !link->I || !link->omega || !link->alpha || !link->a ||
          !link->omega_prev || !link->f_next || !link->fnet || !link->tnet ||
          !link->f || !link->t) {
        syslog(LOG_ERR, "%s: Out of memory.", __func__);
        bt_dynamics_destroy(dyn);
        return -1;
...
static int eval_inverse_forward(struct bt_dynamics *dyn,
  ...
  gsl_blas_dgemv(CblasTrans, 1.0, kin_link->rot_to_prev, link->prev->omega, 0.0,
                 link->omega);
  ...
  gsl_blas_dgemv(CblasTrans, 1.0, kin_link->rot_to_prev, link->prev->alpha, 0.0,
                 link->alpha);
...
int bt_dynamics_eval_jsim(struct bt_dynamics *dyn, struct bt_kinematics *kin) {
  ...
  for (j = 0; j < dyn->dof; j++) {
    ...
    gsl_blas_dgemv(CblasNoTrans, 1.0, kin_link->rot_to_world, link->com, 0.0,
                   dyn->temp1_v3);

    /* Evaluate the jacobian at the link's COM point */
    bt_kinematics_eval_jacobian(kin, j + 1, dyn->temp1_v3, link->com_jacobian);

    /* Add in the linear velocity component */
    gsl_blas_dgemm(CblasTrans, CblasNoTrans, link->mass,
                   link->com_jacobian_linear, link->com_jacobian_linear, 1.0,
                   dyn->jsim);
    ...
    gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, link->com_jacobian_angular,
                   dyn->temp3xn_1, 1.0, dyn->jsim);
  }
