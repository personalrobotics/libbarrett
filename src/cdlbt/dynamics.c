...
static int eval_inverse_forward(struct bt_dynamics *dyn,
                                ...
                                struct bt_kinematics_link *kin_link, double vel,
                                double acc);
...
int bt_dynamics_create(struct bt_dynamics **dynptr, config_setting_t *dynconfig,
	...
	dyn->link_array = (struct bt_dynamics_link **)malloc(
	    dyn->nlinks * sizeof(struct bt_dynamics_link *));
	if (!dyn->link_array) {
		syslog(LOG_ERR, "%s: Out of memory.", __func__);
	...
	for (i = 0; i < dyn->nlinks; i++) {
		struct bt_dynamics_link *link;
		link =
		    (struct bt_dynamics_link *)malloc(sizeof(struct bt_dynamics_link));
		if (!link) {
			syslog(LOG_ERR, "%s: Out of memory.", __func__);
			bt_dynamics_destroy(dyn);
			return -1;
	...
	if (!(moving = config_setting_get_member(dynconfig, "moving")) ||
		...
		syslog(LOG_ERR, "%s: dyn:moving not a list with %d elements.", __func__,
		       ndofs);
		bt_dynamics_destroy(dyn);
		return -1;
	}
	...
	for (i = 0; i < dyn->nlinks; i++) {
		...
		if (link == dyn->base) {
			...
			/* Do memory checks */
			if (!link->com || !link->I || !link->omega || !link->alpha ||
			    !link->a || !link->omega_prev || !link->f_next || !link->fnet ||
			    !link->tnet || !link->f || !link->t) {
				syslog(LOG_ERR, "%s: Out of memory.", __func__);
				bt_dynamics_destroy(dyn);
				return -1;
			}
			...
			/* Allocate matrix views into COM jacobian */
			{
				...
				view =
				    gsl_matrix_submatrix(link->com_jacobian, 0, 0, 3, dyn->dof);
				*(link->com_jacobian_linear) = view.matrix;

				link->com_jacobian_angular =
				    (gsl_matrix *)malloc(sizeof(gsl_matrix));
				...
				view =
				    gsl_matrix_submatrix(link->com_jacobian, 3, 0, 3, dyn->dof);
				*(link->com_jacobian_angular) = view.matrix;
			}

			/* Read the mass */
			err = bt_gsl_config_double_from_group(link_grp, "mass",
			                                      &(link->mass));
			if (err) {
				syslog(LOG_ERR, "%s: No mass in link, or not a number.",
				       __func__);
				bt_dynamics_destroy(dyn);
				return -1;
			}
			...
			if (err) {
				syslog(LOG_ERR, "%s: No com in link, or not a number.",
				       __func__);
				bt_dynamics_destroy(dyn);
				return -1;
			}
...
int bt_dynamics_eval_inverse(struct bt_dynamics *dyn,
	...
	for (j = 0; j < dyn->dof; j++) {
		eval_inverse_forward(dyn, dyn->link[j], kin->link[j],
		                     gsl_vector_get(jvel, j), gsl_vector_get(jacc, j));
	}
...
static int eval_inverse_forward(struct bt_dynamics *dyn,
	...
	gsl_blas_dgemv(CblasTrans, 1.0, kin_link->rot_to_prev, link->prev->omega,
	               0.0, link->omega);
	 ...
	 * omega += qdot_j axis */
	gsl_blas_daxpy(vel, kin_link->prev_axis_z, link->omega);
	...
	gsl_blas_dgemv(CblasTrans, 1.0, kin_link->rot_to_prev, link->prev->alpha,
	               0.0, link->alpha);

	/* Next, add in my joint's acceleration
	 * alpha += qdotdot_j axis */
	gsl_blas_daxpy(acc, kin_link->prev_axis_z, link->alpha);
	...
	gsl_blas_daxpy(vel, kin_link->prev_axis_z, dyn->temp2_v3);
	bt_gsl_cross(link->omega_prev, dyn->temp2_v3, link->alpha);
	...
	gsl_blas_dgemv(CblasTrans, 1.0, kin_link->rot_to_prev, dyn->temp1_v3, 0.0,
	               link->a);
...
static int eval_inverse_backward(struct bt_dynamics *dyn,
	 ...
	 * tnet = I alpha */
	gsl_blas_dgemv(CblasNoTrans, 1.0, link->I, link->alpha, 0.0, link->tnet);

	/* Last, copy in the weird velocity terms
	 * tnet += omega x (I omega) */
	gsl_blas_dgemv(CblasNoTrans, 1.0, link->I, link->omega, 0.0, dyn->temp2_v3);
	...
	if (link->next) {
		...
		gsl_blas_dgemv(CblasNoTrans, 1.0, kin_link->next->rot_to_prev,
		               link->next->f, 0.0, link->f_next);
		gsl_blas_daxpy(1.0, link->f_next, link->f);
	}
	...
	if (link->next) {
		...
		bt_gsl_cross(kin_link->next->prev_origin_pos, link->f_next, link->t);
	}

	/* Get the component of the torque in the axis direction */
	gsl_blas_ddot(kin_link->prev_axis_z, link->t, torque);
...
int bt_dynamics_eval_jsim(struct bt_dynamics *dyn, struct bt_kinematics *kin) {
	...
	for (j = 0; j < dyn->dof; j++) {
		...
		gsl_blas_dgemv(CblasNoTrans, 1.0, kin_link->rot_to_world, link->com,
		               0.0, dyn->temp1_v3);
		...
		gsl_blas_dgemm(CblasTrans, CblasNoTrans, link->mass,
		...
		/* Do I x R^T */
		gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, link->I,
		               kin_link->rot_to_world, 0.0, dyn->temp3x3_1);
		/* Do R x (IxR^T) (AAH OVERLAP!) */
		gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, kin_link->rot_to_world,
		               dyn->temp3x3_1, 0.0, dyn->temp3x3_2);
		/* Do (RxIxR^T) x Jw */
		gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, dyn->temp3x3_2,
		               link->com_jacobian_angular, 0.0, dyn->temp3xn_1);
		/* Do Jw^T x (RxIxR^TxJw) */
		gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0,
		               link->com_jacobian_angular, dyn->temp3xn_1, 1.0,
		               dyn->jsim);
	}
