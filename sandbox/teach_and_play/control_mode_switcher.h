...
template <size_t DOF> class ControlModeSwitcher {
  public:
	BARRETT_UNITS_TEMPLATE_TYPEDEFS(DOF);

	enum ControlMode { CURRENT, VOLTAGE };

	ControlModeSwitcher(barrett::ProductManager &pm_,
	                    barrett::systems::Wam<DOF> &wam_,
	                    const libconfig::Setting &setting_)
	    : pm(pm_), wam(wam_), setting(setting_), mode(VOLTAGE),
	      cGain(setting["current_gain"]), vGain(setting["voltage_gain"]),
	      torqueGainSys(vGain) {
	...
	void currentControl() {
		...
		for (size_t i = 0; i < DOF; ++i) {
			wam.getLowLevelWam().getPucks()[i]->resetProperty(
			    barrett::Puck::IKCOR);
			wam.getLowLevelWam().getPucks()[i]->resetProperty(
			    barrett::Puck::IKP);
			wam.getLowLevelWam().getPucks()[i]->resetProperty(
		...
		const libconfig::Setting &wamSetting =
		...
		wam.tpoToController.setFromConfig(
		    wamSetting["tool_orientation_control"]);
	...
	void voltageControl() {
		...
		wam.getLowLevelWam().getPuckGroup().setProperty(barrett::Puck::IKP,
		                                                8000);
		wam.getLowLevelWam().getPuckGroup().setProperty(barrett::Puck::IKCOR,
		                                                0);

		wam.jpController.setFromConfig(
		    setting["voltage_joint_position_control"]);
		wam.jpController.resetIntegrator();

		wam.tpController.setFromConfig(
		    setting["voltage_tool_position_control"]);
		wam.tpController.resetIntegrator();
		wam.tpoTpController.setFromConfig(
		    setting["voltage_tool_position_control"]);
		wam.tpoTpController.resetIntegrator();

		wam.toController.setFromConfig(
		    setting["voltage_tool_orientation_control"]);
		wam.tpoToController.setFromConfig(
		    setting["voltage_tool_orientation_control"]);
	...
	void calculateTorqueGain() {
		...
		libconfig::Setting &setting =
		    config.lookup("gravitycal")[pm.getWamDefaultConfigPath()];
		...
		for (int i = 0; i < setting.getLength(); ++i) {
			...
			jt_type gravity =
			    wam.jtSum.getInput(barrett::systems::Wam<DOF>::GRAVITY_INPUT)
			        .getValue();
			jt_type supporting = wam.llww.input.getValue();
			double scale =
			    (gravity.dot(supporting) / gravity.norm()) / gravity.norm();
