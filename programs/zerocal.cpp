...
const std::string DATA_CONFIG_FILE =
    barrett::EtcPathRelative("calibration_data/%s/zerocal.conf");
...
class ScopedAttr {
  public:
	...
	explicit ScopedAttr(unsigned long attr, bool startOn = false)
	    : isOn(false), a(-1) {
		set(attr, startOn);
	}
	~ScopedAttr() { off(); }
...
class CalibrationStep {
  public:
	...
	virtual bool onKeyPress(enum Key k) { return true; }
	...
	static int height(int origTop) { return getcury(stdscr) - origTop + 1; }
...
// particular joint.
template <size_t DOF> class AdjustJointStep : public CalibrationStep {
	BARRETT_UNITS_TEMPLATE_TYPEDEFS(DOF);

	static const int DEFAULT_DIGIT =
	    -2;                         // Start by adjusting the hundredth's place.
	static const int MAX_DIGIT = 0; // Display 1's...
	static const int MIN_DIGIT = -3; // ... through thousandth's.

  public:
	explicit AdjustJointStep(const libconfig::Setting &setting,
	                         systems::Wam<DOF> *wamPtr, jp_type *calOffsetPtr,
	                         jp_type *zeroPosPtr, v_type *zeroAnglePtr)
	    : CalibrationStep("Joint " +
	                      boost::lexical_cast<std::string>((int)setting[0])),
	      j((int)setting[0] - 1), calPos(setting[1]),
	      endCondition(setting[2].c_str()), wam(*wamPtr),
	      calOffset(*calOffsetPtr), zeroPos(*zeroPosPtr),
	      zeroAngle(*zeroAnglePtr), state(0), digit(DEFAULT_DIGIT) {
		assert(j >= 0);
		assert(j < DOF);
		assert(wamPtr != NULL);
		...
		default:
			...
			case K_ENTER:
				if (wam.moveIsDone()) {
					...
					// Record actual joint position, not commanded joint
					// position Note: Change added from zerocal branch upstream
					zeroPos[j] = wam.getJointPositions()[j] - calPos[j];

					// Record the motor angles that affect this joint
					const sqm_type &m2jp =
					    llw.getMotorToJointPositionTransform();
					double tolerance = m2jp.cwiseAbs().maxCoeff() * 1e-5;
					for (size_t i = 0; i < DOF; ++i) {
						// If the j,i entry is non-zero, then Motor i is in some
						// way connected to Joint j
						if (math::abs(m2jp(j, i)) > tolerance) {
							int mech =
							    llw.getPucks()[i]->getProperty(Puck::MECH);
							zeroAngle[i] =
							    llw.getMotorPucks()[i].counts2rad(mech);
						}
					}

					// Log joint encoder data for factory calibration
					if (llw.hasJointEncoders()) {
						logMessage("Joint encoder angles: %s") %
						    llw.getJointPositions(
	...
	virtual int display(int top, int left) {
		...
		if (active) {
			...
			// Prompt to move the WAM
			case 0:
				mvprintw(line++, left, "Press [Enter] to move to: [%0.2f",
				         calPos[0]);
			...
			default:
				mvprintw(line++, left,
				         "Use arrow keys to adjust the Joint %d offset.",
				         j + 1);
				sa.off();
				line++;
				mvprintw(line++, left, "                 ");
				...
				printw("Offset (rad)");
				attroff(A_UNDERLINE);
				mvprintw(line, left, "                    %+06.3f",
				         calOffset[j]);

				int x = getcurx(stdscr);
				mvchgat(line, x - 5 - digit + ((digit < 0) ? 1 : 0), 1,
				        A_STANDOUT, 0, NULL);

				line += 2;
				mvprintw(line++, left, "Press [Enter] once %s.",
				         endCondition.c_str());
				break;
			}
		}
...
class ExitStep : public CalibrationStep {
  public:
	explicit ExitStep(const std::string &title_ = "Exit")
	    : CalibrationStep(title_) {}
	virtual ~ExitStep() {}

	virtual bool onKeyPress(enum Key k) { return k != K_ENTER; }
	...
	virtual int display(int top, int left) {
		...
		if (active) {
			ScopedAttr sa(A_STANDOUT, true);
			mvprintw(line++, left,
			         "Press [Enter] to move the home position and record "
			         "calibration data.");
		}
...
int wam_main(int argc, char **argv, ProductManager &pm,
	...
	try {
		...
		const libconfig::Setting &setting =
		    config.lookup("zerocal")[pm.getWamDefaultConfigPath()];
		assert(setting.isList());
		assert(setting.getLength() == (int)DOF);

		for (size_t i = 0; i < DOF; ++i) {
			// Populate the menu
			steps.push_back(new AdjustJointStep<DOF>(
			    setting[i], &wam, &calOffset, &zeroPos, &zeroAngle));
		}
	} catch (libconfig::ParseException e) {
		printf(">>> CONFIG FILE ERROR on line %d of %s: \"%s\"\n", e.getLine(),
		       CAL_CONFIG_FILE.c_str(), e.getError());
		return 1;
	...
	if (done) {
		...
		libconfig::Setting &homeSetting =
		    dataConfig.getRoot().add("home", libconfig::Setting::TypeList);
		libconfig::Setting &zaSetting =
		    dataConfig.getRoot().add("zeroangle", libconfig::Setting::TypeList);
		for (size_t i = 0; i < DOF; ++i) {
		...
		printf(
		    ">>> ERROR: WAM was Idled before the calibration was completed.\n");
		return 1;
	}
...
int main(int argc, char **argv) {
	...
	printf("\n"
	       ...
	       "\n"
	       "This utility will help you set the joint angle offsets for your "
	       "WAM Arm. Joint\n"
	       "angle offsets affect operations that rely on the robot's "
	       "kinematics, such as\n"
	       "gravity compensation, haptics, and Cartesian position and "
	       "orientation control.\n"
	       "You should perform this calibration any time you replace a broken "
	       "cable, add\n"
	       "tension to a cable, or re-mount a Puck.\n"
	       "\n"
	       "The WAM's standard home position is described in the User Manual. "
	       "However, you\n"
	       "may choose any other pose to be your home position if it is more "
	       "convenient for\n"
	       "your application. You will need to return the WAM to the home "
	       "position on power-\n"
	       "up and after certain types of safety faults.\n"
	       "\n"
	       "Modern WAMs have absolute encoders on their motors, so the WAM "
	       "only needs to be\n"
	       "returned to within one motor revolution of its home position. This "
	       "translates to\n"
	       "roughly +/- 5 degrees at the joint. (The tolerance is different "
	       "for each joint.)\n"
	       "\n"
	...
	libconfig::Setting &llSetting =
	    pm.getConfig().lookup(pm.getWamDefaultConfigPath())["low_level"];
	...
	printf(">>> Please *carefully* place the WAM in its home position, then "
	       "press [Enter].");
	waitForEnter();
