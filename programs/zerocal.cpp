  ...
  public:
	...
	virtual int display(int top, int left) {
		...
		if (active) {
			ScopedAttr sa(A_STANDOUT, true);
			mvprintw(line++, left,
			         "Press [Enter] to move the home position and "
...
int main(int argc, char **argv) {
	...
	printf(
	    ...
	    "\n"
	    "This utility will help you set the joint angle offsets for your WAM "
	    "Arm. Joint\n"
	    "angle offsets affect operations that rely on the robot's kinematics, "
	    "such as\n"
	    ...
	    "\n"
	    "Modern WAMs have absolute encoders on their motors, so the WAM only "
	    "needs to be\n"
	    "returned to within one motor revolution of its home position. This "
	    "translates to\n"
	    "roughly +/- 5 degrees at the joint. (The tolerance is different for "
	    "each joint.)\n"
	    "\n"
