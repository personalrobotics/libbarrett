...
public:
	  ...
	  case K_ENTER:
		if (wam.moveIsDone()) {
		  ...
		  for (size_t i = 0; i < DOF; ++i) {
			// If the j,i entry is non-zero, then Motor i is in some way
		  ...
		  if (llw.hasJointEncoders()) {
			logMessage("Joint encoder angles: %s") %
			    llw.getJointPositions(LowLevelWam<DOF>::PS_JOINT_ENCODER);
		  }
  ...
  virtual int display(int top, int left) {
	...
	if (active) {
	  ...
	  // Adjust the offset
	  default:
		mvprintw(line++, left, "Use arrow keys to adjust the Joint %d offset.",
		         j + 1);
		sa.off();
		line++;
		mvprintw(line++, left, "                 ");
		...
		mvchgat(line, x - 5 - digit + ((digit < 0) ? 1 : 0), 1, A_STANDOUT, 0,
		        NULL);
...
public:
  ...
  virtual int display(int top, int left) {
	...
	if (active) {
	  ScopedAttr sa(A_STANDOUT, true);
	  mvprintw(line++, left, "Press [Enter] to move the home position and "
...
int wam_main(int argc, char **argv, ProductManager &pm,
  ...
  try {
	...
	for (size_t i = 0; i < DOF; ++i) {
	  // Populate the menu
	  steps.push_back(new AdjustJointStep<DOF>(setting[i], &wam, &calOffset,
	                                           &zeroPos, &zeroAngle));
	}
