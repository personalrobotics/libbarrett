...
bool validate_args(int argc, char **argv) {
  printf(
      ...
      "each link\n"
      "of your WAM Arm. This data is used by the gravity compensation routine "
      "to\n"
      ...
      "proceeding. It\n"
      "is also necessary to know how the WAM is oriented relative to gravity, "
      "so be\n"
...
int wam_main(int argc, char **argv, ProductManager &pm,
  ...
  if (done == 1) {
	...
	bt_calgrav_create(&grav, wamSetting["gravity_compensation"].getCSetting(),
	                  n);
	...
	for (pose = 0; pose < num_poses; pose++) {
	  ...
	  for (j = 0; j < n; j++) {
		...
		/* GT: -R*L */
		gsl_matrix_set(GT[j], 3 * pose + 0, 3 + 2 * pose + 0,
		               -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 0));
		gsl_matrix_set(GT[j], 3 * pose + 0, 3 + 2 * pose + 1,
		               -gsl_matrix_get(kin->link[j]->rot_to_prev, 1, 0));
		gsl_matrix_set(GT[j], 3 * pose + 1, 3 + 2 * pose + 0,
		               -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 1));
		gsl_matrix_set(GT[j], 3 * pose + 1, 3 + 2 * pose + 1,
		               -gsl_matrix_get(kin->link[j]->rot_to_prev, 1, 1));
		gsl_matrix_set(GT[j], 3 * pose + 2, 3 + 2 * pose + 0,
		               -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 2));
		gsl_matrix_set(GT[j], 3 * pose + 2, 3 + 2 * pose + 1,
	...
	/* Do the regression for each link */
	{
	  ...
	  for (j = n - 1; j >= 0; j--) {
		...
		gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, m2x2inverse, GT[j], 0.0,
		               m2x3);
