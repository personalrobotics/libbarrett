...
const std::string CAL_CONFIG_FILE =
    barrett::EtcPathRelative("calibration.conf");
const std::string DATA_CONFIG_FILE =
    barrett::EtcPathRelative("calibration_data/%s/gravitycal.conf");
...
bool validate_args(int argc, char **argv) {
	printf(
	    ...
	    "\n"
	    "This utility will calculate cumulative first moment of mass data for "
	    "each link\n"
	    "of your WAM Arm. This data is used by the gravity compensation "
	    "routine to\n"
	    "support WAM's weight in gravity. The program will move the WAM to "
	    "several\n"
	    "predefined positions and take torque measurements at each location.\n"
	    "\n"
	    "The calculations rely on having accurate kinematic information. "
	    "Consider\n"
	    "performing the zero-calibration procedure (bt-wam-zerocal) before "
	    "proceeding. It\n"
	    "is also necessary to know how the WAM is oriented relative to "
	    "gravity, so be\n"
	    "sure to update the \"world_to_base\" transform if your WAM is not "
	    "mounted in the\n"
	    "standard orientation.\n"
	    "\n"
	    "\n"
	    "IMPORTANT: DO NOT TOUCH the WAM during the measurement process, or "
	    "the\n"
	    "calibration computations will be significantly wrong, and any "
	    "subsequent gravity\n"
...
int done;
void sigint(int) { done = -1; }

template <size_t DOF>
int mu_callback(
    const boost::tuple<typename units::JointTorques<DOF>::type,
                       typename units::JointPositions<DOF>::type> &t) {
	size_t j;
...
int mu_stats(gsl_vector *torques, gsl_vector *positions) {
	...
	for (j = 0; j < dof; j++) {
		...
		mvprintw(13, 13 + 9 * j, "% 08.5f ",
		         sqrt(gsl_vector_get(position_vars, j)));
		mvprintw(14, 13 + 9 * j, "% 08.5f ", gsl_vector_get(torques, j));
		mvprintw(15, 13 + 9 * j, "% 08.5f ",
		         sqrt(gsl_vector_get(torque_vars, j)));
	}
...
int wam_main(int argc, char **argv, ProductManager &pm,
	...
	// The longest string below is 9 characters (including the NULL
	// termination).
	char phasenm[][9] = {"START",  "TO_TOP",   "FROM_TOP", "MEAS_TOP",
	                     "TO_BOT", "FROM_BOT", "MEAS_BOT", "DONE"};
	 ...
	 * (fills poses, sets num_poses) */
	{
		...
		if (err != CONFIG_TRUE) {
			syslog(LOG_ERR, "Calibration configuration file %s not found.",
		...
		for (pose = 0; pose < num_poses; pose++) {
			...
			err = bt_gsl_fill_vector_cfgarray(
			    poses[pose], config_setting_get_elem(poses_array, pose));
			if (err) {
	...
	systems::Callback<boost::tuple<jt_type, jp_type>, int> muCallback(
	    mu_callback<DOF>);
	pm.getExecutionManager()->startManaging(
	    muCallback); // Make sure mu_callback() is called every execution cycle
	...
	wam.jpController.setControlSignalLimit(
	    jp_type()); // Disable torque saturation because gravity comp isn't on
	...
	clear();
	mvprintw(1, 0,
	         "Note: Press [Control+C] at any time to cancel the calibration.");
	...
	while (!done) {
		...
		for (j = 0; j < n; j++) {
			...
			mvprintw(5, 13 + 9 * j, "% 08.5f ",
			         gsl_vector_get(wam.getJointPositions().asGslType(), j));
			mvprintw(6, 13 + 9 * j, "% 08.4f ",
			         gsl_vector_get(wam.llww.input.getValue().asGslType(), j));
		}
		...
		case MU_P_START:
			...
			syslog(LOG_ERR, "Pose is: %s",
			       bt_gsl_vector_sprintf(buf, poses[pose]));
			gsl_blas_dcopy(poses[pose], moveto_vec);
			gsl_blas_daxpy(1.0, angle_diff, moveto_vec);
			syslog(LOG_ERR, "Moving to: %s",
			       bt_gsl_vector_sprintf(buf, moveto_vec));
	...
	if (done == 1) {
		...
		barrett::detail::getCSetting(
		    bt_kinematics_create(&kin, setting["kinematics"]), n);
		barrett::detail::getCSetting(
		    bt_calgrav_create(&grav, setting["gravity_compensation"]), n);
		...
		for (pose = 0; pose < num_poses; pose++) {
			...
			for (j = 0; j < n; j++) {
				...
				gsl_matrix_set(GT[j], 3 * pose + 0, 1,
				               -gsl_vector_get(grav->g[j], 2));
				gsl_matrix_set(GT[j], 3 * pose + 0, 2,
				               gsl_vector_get(grav->g[j], 1));
				gsl_matrix_set(GT[j], 3 * pose + 1, 0,
				               gsl_vector_get(grav->g[j], 2));
				gsl_matrix_set(GT[j], 3 * pose + 1, 2,
				               -gsl_vector_get(grav->g[j], 0));
				gsl_matrix_set(GT[j], 3 * pose + 2, 0,
				               -gsl_vector_get(grav->g[j], 1));
				gsl_matrix_set(GT[j], 3 * pose + 2, 1,
				               gsl_vector_get(grav->g[j], 0));
				/* GT: -R*L */
				gsl_matrix_set(
				    GT[j], 3 * pose + 0, 3 + 2 * pose + 0,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 0));
				gsl_matrix_set(
				    GT[j], 3 * pose + 0, 3 + 2 * pose + 1,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 1, 0));
				gsl_matrix_set(
				    GT[j], 3 * pose + 1, 3 + 2 * pose + 0,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 1));
				gsl_matrix_set(
				    GT[j], 3 * pose + 1, 3 + 2 * pose + 1,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 1, 1));
				gsl_matrix_set(
				    GT[j], 3 * pose + 2, 3 + 2 * pose + 0,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 0, 2));
				gsl_matrix_set(
				    GT[j], 3 * pose + 2, 3 + 2 * pose + 1,
				    -gsl_matrix_get(kin->link[j]->rot_to_prev, 1, 2));
				/* Y */
				gsl_vector_set(
				    Y[j], 3 * pose + 0,
				    gsl_vector_get(torques[pose], j) *
				        gsl_matrix_get(kin->link[j]->rot_to_prev, 2, 0));
				gsl_vector_set(
				    Y[j], 3 * pose + 1,
				    gsl_vector_get(torques[pose], j) *
				        gsl_matrix_get(kin->link[j]->rot_to_prev, 2, 1));
				gsl_vector_set(
				    Y[j], 3 * pose + 2,
				    gsl_vector_get(torques[pose], j) *
				        gsl_matrix_get(kin->link[j]->rot_to_prev, 2, 2));
				if (j < n - 1)
					gsl_vector_set(Y[j], 3 * pose + 2,
					               gsl_vector_get(Y[j], 3 * pose + 2) -
					                   gsl_vector_get(torques[pose], j + 1));
			}
		}
		...
		/* Do the regression for each link */
		{
			...
			m2x2inverse =
			    gsl_matrix_alloc(3 + 2 * num_poses, 3 + 2 * num_poses);
			...
			for (j = n - 1; j >= 0; j--) {
				...
				/* m2x2 = GT[j]' * GT[j] */
				gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, GT[j], GT[j], 0.0,
				               m2x2);
				...
				gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0, m2x2inverse,
				               GT[j], 0.0, m2x3);
		...
		char *dataConfigFile =
		    new char[strlen(DATA_CONFIG_FILE.c_str()) +
		             strlen(pm.getWamDefaultConfigPath()) - 2 + 1];
		sprintf(dataConfigFile, DATA_CONFIG_FILE.c_str(),
		        pm.getWamDefaultConfigPath());
		manageBackups(dataConfigFile); // Backup old calibration data

		// Save to the data config file
		libconfig::Config dataConfig;
		libconfig::Setting &musSetting =
		    dataConfig.getRoot()
		        .add("gravity_compensation", libconfig::Setting::TypeGroup)
		        .add("mus", libconfig::Setting::TypeList);
		for (size_t i = 0; i < DOF; ++i) {
			libconfig::Setting &rowSetting =
			    musSetting.add(libconfig::Setting::TypeList);
			rowSetting.add(libconfig::Setting::TypeFloat) =
			    gsl_vector_get(mus[i], 0);
			rowSetting.add(libconfig::Setting::TypeFloat) =
			    gsl_vector_get(mus[i], 1);
			rowSetting.add(libconfig::Setting::TypeFloat) =
			    gsl_vector_get(mus[i], 2);
		}
