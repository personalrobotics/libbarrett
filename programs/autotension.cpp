  ...
  if (jointsToAutotension.size() == 0) {
	...
	for (size_t i = 0; i < jointsToAutotension.size(); i++) {
	  if (jointsToAutotension[i] < 1 || jointsToAutotension[i] > jnts2Tens) {
		printf("\nEXITING: Arguments out of range - Must provide valid joints "
		       "1-%d (ex. 1 2 4)\n\n",
		       (DOF == 4) ? 4 : 6);
...
public:
  ...
  AutoTension(systems::Wam<DOF> &wam_, const libconfig::Setting &setting_,
      ...
      : wam(wam_), setting(setting_), armToAutotension(armToAutotension_),
        ...
        jpStopHigh(setting["jp_stop_high"]), jpStopLow(setting["jp_stop_low"]),
        tangBuffer(setting["tang_buffer"]), tangMiss(setting["tang_miss"]),
        stopBuffer(setting["stop_buffer"]),
...
std::vector<int> AutoTension<DOF>::tensionJoint(std::vector<int> joint_list) {
  ...
  while ((motorSlackPulled > slackThreshold[motor] && slackDifference > 1e-3) ||
         !diff_tens) {
	...
	int rep_cnt = 0;
	int reps =
	    5; // Number of times to travel through joint range to distribute slack.
  ...
  default:
	...
	joint_list.erase(std::remove(joint_list.begin(), joint_list.end(), joint),
	                 joint_list.end());
	break;
  }
  return joint_list;
...
template <size_t DOF> bool AutoTension<DOF>::engage(int motor, double timeout) {
  ...
  motorRamp.setOutput((j2mp * wam.getJointPositions())[motor]); // Set the ramp
                                                                // to the proper
                                                                // start
                                                                // position
...
int main(int argc, char **argv) {
  ...
  printf(
      ...
      "\n"
      "WAMs with serial numbers < 5 and WAM wrists with serial numbers < 9 are "
      "not\n"
