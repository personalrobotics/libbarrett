...
template <size_t DOF> std::vector<int> validate_args(int argc, char **argv) {
...
public:
  ...
  AutoTension(systems::Wam<DOF> &wam_, const libconfig::Setting &setting_)
      : wam(wam_), setting(setting_), puck(wam.getLowLevelWam().getPucks()),
  ...
  void updateJ1Moves(int tensionMotor, bool addJ1) {
    if (addJ1) {
      ...
      jpSlack1[tensionMotor][0] =
          jpStart[tensionMotor][0] - tangBuffer[0] / 2.0;
...
void AutoTension<DOF>::init(ProductManager &pm, std::vector<int> args) {
  ...
  jpSlack1[1][1] =
      jpStopHigh[1] -
      stopBuffer[1]; // This is so we dont drive into and wear the joint stops
...
std::vector<int> AutoTension<DOF>::tensionJoint(std::vector<int> joint_list) {
         ...
         !diff_tens) // Check to see if we have met the slack thresholds
  {
    ...
    if (j1SlackPulled < slackThreshold[0]) {
      ...
      joint_list.erase(std::remove(joint_list.begin(), joint_list.end(), 1),
                       joint_list.end());
      j1tens = false;
    }
  }
...
template <size_t DOF> bool AutoTension<DOF>::engage(int motor, double timeout) {
  ...
  motorRamp.setOutput(
      (j2mp * wam.getJointPositions())[motor]); // Set the ramp to the proper
                                                // start position
  ...
  systems::disconnect(
      motorRamp.output); // disconnect the ramp leaving the commanded motor
